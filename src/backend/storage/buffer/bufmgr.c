	...
	Buffer buffer, const ClusterPcmOwnSnapshot *expected)
{
	...
	if (entry == NULL || entry->phase != PCM_X_WRITER_LEDGER_ACTIVE
		...
		|| live.generation
			   != entry->authority.buffer_ownership_generation)
		return false;
	result = cluster_pcm_lock_resource_x_recycler_arm_exact(
		&entry->authority.ref, entry->authority.r4_record_generation,
		expected->generation, expected->reservation_token,
		(uint32) MyProc->pgprocno, &entry->recycler);
	if (result != RESOURCE_X_APPLY_APPLIED)
		return false;
...
cluster_bufmgr_itl_recycle_guard_unlock(Buffer buffer)
{
	...
	if (entry == NULL || entry->phase != PCM_X_WRITER_LEDGER_RECYCLING
		|| !cluster_bufmgr_pcm_x_writer_entry_exact(entry, buf)
		|| entry->recycler.kind != RESOURCE_X_LOCAL_OWNER_RECYCLING
...
cluster_bufmgr_itl_recycle_guard_relock(Buffer buffer)
{
	...
	if (cluster_bufmgr_pcm_own_snapshot(buf, &live) != CLUSTER_PCM_OWN_OK
		|| memcmp(&live, &entry->granted, sizeof(live)) != 0
		|| cluster_pcm_lock_resource_x_recycler_revalidate_exact(
			&entry->recycler) != RESOURCE_X_APPLY_APPLIED)
	{
		(void) cluster_pcm_lock_resource_x_recycler_cancel_exact(
			&entry->recycler);
		cluster_bufmgr_pcm_x_writer_clear(entry);
		LWLockRelease(content_lock);
		return false;
	}
	result = cluster_pcm_lock_resource_x_recycler_release_exact(
		&entry->recycler);
	if (result != RESOURCE_X_APPLY_APPLIED)
	{
		cluster_bufmgr_pcm_x_writer_clear(entry);
...
cluster_bufmgr_itl_recycle_guard_cancel(Buffer buffer)
{
	...
	result = cluster_pcm_lock_resource_x_recycler_cancel_exact(
		&entry->recycler);
	if (result != RESOURCE_X_APPLY_APPLIED
		&& result != RESOURCE_X_APPLY_NOT_FOUND
		&& result != RESOURCE_X_APPLY_STALE)
