...
/* Caller holds the BufferDesc header lock.  The sidecar REVOKING flag and
 * the first shared-refcount increment are therefore one atomic choice. */
static inline bool
cluster_bufmgr_pcm_aux_pin_admission_locked(BufferDesc *buf)
{
	Assert(buf != NULL);
	Assert(pg_atomic_read_u32(&buf->state) & BM_LOCKED);
	return cluster_pcm_x_aux_pin_admission_allowed(
		cluster_pcm_is_active(), cluster_bufmgr_should_pcm_track(buf),
		buf->tag.forkNum, cluster_pcm_own_flags_get(buf->buf_id));
}
...
/* Physical half of the former-source local reacquire interlock.  The
 * retained Resource-X pair is checked in its own lock domain by the caller;
 * this predicate proves that the same sampled generation still names the
 * clean, valid N+PI carrier on either side of SourceSettlement's exact fence
 * release.  REVOKING is the pre-release half; flags==0 is accepted only as
 * the post-release/pre-entry-commit half while the caller still proves the
 * exact undrained pair. */
bool
	...
	BufferDesc *buf, const ClusterPcmOwnSnapshot *expected_n)
{
	...
	if (buf == NULL || expected_n == NULL
		|| expected_n->generation == 0
		|| expected_n->generation == UINT64_MAX
		|| expected_n->reservation_token == 0
...
static bool
cluster_bufmgr_pcm_direct_init_known_new_locked(BufferDesc *buf,
											 uint32 buf_state)
{
	uint32 flags;
	uint64 reservation_token;
	...
	bool aux_pin_handoff_protected = false;
	bool io_known_new;
	bool valid_aux_known_new;

	Assert(buf != NULL);
	Assert(buf_state & BM_LOCKED);
	io_known_new = (buf_state & BM_VALID) == 0
		&& (buf_state & BM_IO_IN_PROGRESS) != 0;
	valid_aux_known_new = (buf_state & BM_VALID) != 0
		&& (buf_state & BM_IO_IN_PROGRESS) == 0
		&& (buf->tag.forkNum == VISIBILITYMAP_FORKNUM
			|| buf->tag.forkNum == FSM_FORKNUM);
	if (valid_aux_known_new && BUF_STATE_GET_REFCOUNT(buf_state) == 0)
	{
		flags = cluster_pcm_own_flags_get(buf->buf_id);
		reservation_token
		...
		resource_x_activation_generation
			= cluster_pcm_own_resource_x_activation_generation_get(
				buf->buf_id);
		/* The fresh proof was armed and consumed while pinned.  Only its exact
		 * VM/FSM GRANT_PENDING token, or that token's contiguous T2/T3 sidecar,
		 * may cover the approved pinless network interval.  Mapping eviction
		 * rejects both live shapes, so no raw descriptor reuse is admitted. */
		aux_pin_handoff_protected
			= reservation_token != 0
			  ...
			  && ((flags == PCM_OWN_FLAG_GRANT_PENDING
				  ...
				  || (flags == 0
					  && writer_activation_token == reservation_token
					  && resource_x_activation_generation != UINT64_MAX));
	}
	return buf->buffer_type != (uint8)BUF_TYPE_PI
		...
		&& (buf_state
			& (BM_DIRTY | BM_JUST_DIRTIED | BM_CHECKPOINT_NEEDED
			   | BM_IO_ERROR)) == 0
		&& PageIsNew((Page)BufHdrGetBlock(buf))
		&& (io_known_new || valid_aux_known_new);
}
	...
	BufferDesc *buf, const ClusterPcmOwnSnapshot *expected_pending_n)
{
	...
	if (buf == NULL || expected_pending_n == NULL
		|| expected_pending_n->pcm_state != (uint8)PCM_STATE_N
		|| expected_pending_n->flags != PCM_OWN_FLAG_GRANT_PENDING
		|| expected_pending_n->reservation_token == 0
		|| expected_pending_n->writer_activation_token != 0
		|| expected_pending_n->resource_x_activation_generation != 0)
		return CLUSTER_PCM_OWN_INVALID;
	if (ClusterPcmOwnArray == NULL)
		return CLUSTER_PCM_OWN_NOT_READY;
	buf_state = LockBufHdr(buf);
 ...
 * The resource master remains authoritative on the wire; this predicate
 * proves only that the local resident mirror still has the exact clean,
 * current S tuple named by the authenticated type-17 operation. */
ClusterPcmOwnResult
										 ...
										 ResourceXBufferInstallProof *out_proof)
{
	...
	if (ref == NULL || image == NULL || out_proof == NULL || image->page_bytes == NULL
		|| image->image_length != BLCKSZ || ref->formation == 0
		|| ref->acquisition_generation == 0)
		return RESOURCE_X_BUFFER_CORRUPT;
	if (ClusterPcmOwnArray == NULL)
		return RESOURCE_X_BUFFER_ABSENT;

	/* PageHeader reads require aligned storage.  The carrier's declared proof
	 * must describe its own immutable bytes before any shared page is touched. */
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				{
					memcpy(page, previous.data, BLCKSZ);
...
/* T3 removes the Resource-X generation before the legacy writer token.  The
 * raw sidecar helper owns that store ordering; this wrapper owns descriptor,
 * byte-lock, and exact-acquisition revalidation around it. */
PGRAC_PCM_X_FENCE_TERMINAL_OWNER(T3_CLEAR, ref,
	...
	const ResourceXAcquisitionRef *ref, ResourceXBufferActivationProof *out_proof)
{
	...
	if (!BufferTagsEqual(&buf->tag, &lookup_tag) || (buf_state & BM_VALID) == 0)
		result = RESOURCE_X_BUFFER_ABSENT;
	else if (!cluster_pcm_x_resource_x_t3_snapshot_exact(ref, &expected))
		result = expected.flags != 0 ? RESOURCE_X_BUFFER_BUSY : RESOURCE_X_BUFFER_STALE;
	else if (!cluster_bufmgr_pcm_current_image_locked(buf, buf_state)
			 || (buf_state & BM_IO_ERROR) != 0)
		result = RESOURCE_X_BUFFER_CORRUPT;
	...
	if (!cluster_pcm_own_snapshot_matches_locked(buf, &expected)
		...
		|| live.generation != expected.generation
		|| live.reservation_token != expected.reservation_token
		|| !cluster_pcm_x_resource_x_t3_snapshot_exact(ref, &live)
		|| (buf_state & BM_VALID) == 0)
		result = RESOURCE_X_BUFFER_STALE;
	else if (!cluster_bufmgr_pcm_current_image_locked(buf, buf_state)
			 || (buf_state & BM_IO_ERROR) != 0)
...
/* User-approved PGRAC adaptation for an exact known-new direct-init
 * descriptor.  The durable image is authority proof only: this helper never
 * copies it into the shared page.  It changes only the Resource-X activation
 * sidecar while mapping, ownership, X writer token, known-new bytes, and the
 * original reservation token all still match. */
static ResourceXBufferActivationResult
cluster_bufmgr_pcm_own_direct_init_sidecar_by_tag_exact(
	const ResourceXAcquisitionRef *ref, uint64 expected_generation,
	uint64 expected_reservation_token, bool clear,
	...
	ResourceXBufferActivationProof *activation_proof)
{
	...
	if (install_proof != NULL)
		memset(install_proof, 0, sizeof(*install_proof));
	if (activation_proof != NULL)
		memset(activation_proof, 0, sizeof(*activation_proof));
	if (ref == NULL || ref->formation == 0
		|| ref->acquisition_generation == 0
		|| expected_generation == 0
		|| expected_generation == UINT64_MAX
		|| expected_reservation_token == 0
		|| expected_reservation_token == UINT64_MAX
		|| (clear && activation_proof == NULL)
		|| (!clear && install_proof == NULL))
		return RESOURCE_X_BUFFER_CORRUPT;
	if (ClusterPcmOwnArray == NULL)
		return RESOURCE_X_BUFFER_ABSENT;
	...
	buf_state = LockBufHdr(buf);
	cluster_pcm_own_snapshot_locked(buf, &expected);
	if (!BufferTagsEqual(&buf->tag, &lookup_tag))
		result = RESOURCE_X_BUFFER_ABSENT;
	else if (expected.generation != expected_generation
			 || expected.reservation_token != expected_reservation_token
			 || (clear
				 ? !cluster_pcm_x_resource_x_t3_snapshot_exact(ref, &expected)
				 : !cluster_pcm_x_resource_x_t2_snapshot_exact(ref, &expected)))
		result = expected.flags != 0
			? RESOURCE_X_BUFFER_BUSY : RESOURCE_X_BUFFER_STALE;
	else if (buf->buffer_type != (uint8)BUF_TYPE_XCUR
			 || !cluster_bufmgr_pcm_direct_init_known_new_locked(buf, buf_state))
		result = RESOURCE_X_BUFFER_STALE;
	else
	{
	...
	{
		already_bound = live.resource_x_activation_generation
			== ref->acquisition_generation;
		if (clear)
			own_result = cluster_pcm_own_resource_x_activation_clear_exact(
				buf->buf_id, expected_generation,
		...
		if (own_result == CLUSTER_PCM_OWN_OK)
		{
			...
			if (clear)
			{
				activation_proof->ownership_generation = live.generation;
				activation_proof->writer_activation_token
					= live.writer_activation_token;
				activation_proof->resource_x_activation_generation
					= live.resource_x_activation_generation;
				result = RESOURCE_X_BUFFER_T2_INSTALLED;
			}
			else
			{
				install_proof->ownership_generation = live.generation;
				install_proof->writer_activation_token
					= live.writer_activation_token;
				install_proof->resource_x_activation_generation
					= live.resource_x_activation_generation;
...
{
	return cluster_bufmgr_pcm_own_direct_init_sidecar_by_tag_exact(
		ref, expected_generation, expected_reservation_token, false,
		out_proof, NULL);
}
...
{
	return cluster_bufmgr_pcm_own_direct_init_sidecar_by_tag_exact(
		ref, expected_generation, expected_reservation_token, true,
		NULL, out_proof);
}

/* R8 reconfiguration owner for one exact T2/T3 gap.  It takes only the
 * mapping partition and BufferDesc header lock: no content lock, pin, I/O, or
 * Resource-X entry lock crosses this boundary.  The descriptor becomes a
 * retained non-writable N+PI image before the Resource-X generation and then
 * writer token are cleared. */
PGRAC_PCM_X_FENCE_TERMINAL_OWNER(R8_NEUTRALIZE, tag,
	cluster_pcm_x_resource_x_r8_snapshot_exact)
										   ...
										   uint64 acquisition_generation)
{
	...
	uint32 buf_state;
	int buf_id;
	ResourceXSidecarNeutralizeResult result = RESOURCE_X_SIDECAR_STALE;

	if (tag == NULL || old_formation == 0 || old_formation == UINT64_MAX
		|| acquisition_generation == 0 || acquisition_generation == UINT64_MAX)
		return RESOURCE_X_SIDECAR_CORRUPT;
	if (ClusterPcmOwnArray == NULL)
		return RESOURCE_X_SIDECAR_STALE;
	...
	cluster_pcm_own_snapshot_locked(buf, &live);
	if (!BufferTagsEqual(&buf->tag, &lookup_tag) || (buf_state & BM_VALID) == 0)
		result = RESOURCE_X_SIDECAR_STALE;
	else if (live.resource_x_activation_generation == 0)
		result = live.writer_activation_token == 0 ? RESOURCE_X_SIDECAR_ALREADY_CLEAR
												 : RESOURCE_X_SIDECAR_STALE;
	else if (live.resource_x_activation_generation != acquisition_generation)
		result = RESOURCE_X_SIDECAR_SUCCESSOR;
	else if (!cluster_pcm_x_resource_x_r8_snapshot_exact(
				 tag, old_formation, acquisition_generation, &live))
		result = live.flags != 0 ? RESOURCE_X_SIDECAR_BUSY : RESOURCE_X_SIDECAR_CORRUPT;
	else if ((buf_state & BM_IO_IN_PROGRESS) != 0)
		result = RESOURCE_X_SIDECAR_BUSY;
	else if ((buf_state
			  & (BM_DIRTY | BM_JUST_DIRTIED | BM_CHECKPOINT_NEEDED | BM_IO_ERROR))
			 != 0
			 || !cluster_bufmgr_pcm_current_image_locked(buf, buf_state))
		result = RESOURCE_X_SIDECAR_CORRUPT;
	else if (live.generation == UINT64_MAX)
		result = RESOURCE_X_SIDECAR_CORRUPT;
	else {
		/* Quarantine first.  From this store onward an unexpected raw-sidecar
		 * mismatch retains N+PI plus the live fences; restoring X would reopen
		 * an acquisition the reconfiguration owner could not prove. */
		buf->pcm_state = (uint8)PCM_STATE_N;
		buf->buffer_type = (uint8)BUF_TYPE_PI;
		own_result = cluster_pcm_own_resource_x_neutralize_exact(
			buf->buf_id, live.generation, live.reservation_token,
			acquisition_generation, &neutral_generation);
		if (own_result == CLUSTER_PCM_OWN_OK && neutral_generation != 0)
			result = RESOURCE_X_SIDECAR_NEUTRALIZED;
		else
			result = RESOURCE_X_SIDECAR_CORRUPT;
	}
	UnlockBufHdr(buf, buf_state);
...
/* Resolve the exact pending descriptor named by a requester bootstrap round.
 * Unlike the ordinary resident lookup, the main-fork known-new window is
 * deliberately !BM_VALID while StartBufferIO still owns BM_IO_IN_PROGRESS.
 * The caller must revalidate the returned snapshot before every transition. */
ClusterPcmOwnResult
	...
	ClusterPcmOwnSnapshot *out_snapshot)
{
	...
	if (!BufferTagsEqual(&buf->tag, tag)
		|| out_snapshot->generation != expected_generation
		|| out_snapshot->reservation_token != expected_reservation_token
		|| out_snapshot->flags != PCM_OWN_FLAG_GRANT_PENDING
		|| out_snapshot->pcm_state != (uint8)PCM_STATE_N
		|| out_snapshot->writer_activation_token != 0
		|| out_snapshot->resource_x_activation_generation != 0)
		result = CLUSTER_PCM_OWN_STALE;
	else if (buf->buffer_type != (uint8)BUF_TYPE_CURRENT
			 || !cluster_bufmgr_pcm_direct_init_known_new_locked(buf, buf_state))
		result = CLUSTER_PCM_OWN_STALE;
	else
	{
...
/* Finish the current-slice remote non-requester S-holder adaptation only
 * after its exact type-18 status is retained as an unsendable DATA-ring
 * PENDING slot.  Content EXCLUSIVE and the REVOKING token bind the final
 * clean/current byte state to the sole S->N ownership generation bump. */
ClusterPcmOwnResult
	...
	ClusterPcmOwnSnapshot *out_n_snapshot)
{
	...
	if (buf == NULL || expected_revoking == NULL || out_n_snapshot == NULL)
		return CLUSTER_PCM_OWN_INVALID;
	memset(out_n_snapshot, 0, sizeof(*out_n_snapshot));
	...
	if (!cluster_pcm_own_snapshot_matches_locked(buf, expected_revoking)
		...
		if (live_result == CLUSTER_PCM_OWN_CORRUPT)
			...
			if (result == CLUSTER_PCM_OWN_OK) {
				...
				Assert(committed_generation
					   == expected_revoking->generation + 1);
				buf->pcm_state = (uint8)PCM_STATE_N;
				buf->buffer_type = (uint8)BUF_TYPE_CURRENT;
				cluster_pcm_own_snapshot_locked(buf, out_n_snapshot);
			}
...
static bool cluster_bufmgr_pcm_x_writer_activate(
	ClusterPcmXWriterLedgerEntry *entry, bool allow_reprobe);
										 ...
										 bool *barrier_refused, uint64 *covered_generation)
{
	...
	elog(LOG,
		 ...
		 (unsigned long long)cluster_pcm_own_reservation_token_get(buf->buf_id), current_flags);
	...
	bool *pcm_x_transient_refused)
{
	...
	for (;;)
	{
		...
		tag_current = own_result == CLUSTER_PCM_OWN_OK
			&& BufferTagsEqual(&granted.tag, expected_resource)
			&& BufferTagsEqual(
				&terminal_ref.assertion.resource, expected_resource);
		if (own_result != CLUSTER_PCM_OWN_OK
				...
				&context))
		{
			bool retryable
				= cluster_pcm_x_target_preuse_drift_retryable(
					&granted, path_current, gate_current, tag_current);
...
/* Direct-init obtains its exact TARGET grant before the caller can take the
 * content lock.  Finish the existing TARGET activation under that lock and
 * before the caller may initialize any page bytes.  SOURCE/non-cluster
 * direct-init has no ledger and remains a no-op here. */
static void
cluster_bufmgr_pcm_x_writer_activate_target_direct_init(BufferDesc *buf)
{
	...
	if (entry == NULL)
		return;
	(void) cluster_bufmgr_pcm_x_writer_activate(entry, false);
}
...
cluster_bufmgr_itl_recycle_guard_relock(Buffer buffer)
{
	...
	{
		LWLockRelease(content_lock);
		/* Preserve the saved exact handle until the shared RECYCLING owner has
		 * been cancelled (or an already-frozen HANDOFF is observed as stale).
		 * Clearing the backend ledger first makes the outer no-throw unwind
		 * unable to release the per-resource owner and strands the successor. */
 ...
 * and can restore them only to the same tag in the same descriptor.  The
 * Resource-X round and the later ownership snapshot remain the authority;
 * this object grants nothing and survives no call boundary. */
	...
	ClusterBufmgrPcmAuxPinHandoff *handoff)
{
	...
	if (buf == NULL || expected_resource == NULL || handoff == NULL
	...
	buf_state = LockBufHdr(buf);
	private_refcount = GetPrivateRefCount(buffer);
	if (private_refcount <= 0 || (buf_state & BM_VALID) == 0
	...
	handoff->released_refs = private_refcount;
	UnlockBufHdr(buf, buf_state);

	for (i = 0; i < private_refcount; i++)
...
cluster_bufmgr_pcm_aux_pin_handoff_finish_exact(
	BufferDesc *buf, ClusterBufmgrPcmAuxPinHandoff *handoff)
{
	...
	if (buf == NULL || handoff == NULL || !handoff->active
		|| handoff->buffer != BufferDescriptorGetBuffer(buf)
		|| handoff->released_refs <= 0)
		return false;
	rlocator = BufTagGetRelFileLocator(&handoff->tag);
	while (reacquired < handoff->released_refs)
	{
		if (!ReadRecentBuffer(rlocator,
				BufTagGetForkNum(&handoff->tag), handoff->tag.blockNum,
				handoff->buffer))
			goto repin_failed;
		reacquired++;
	}
	buf_state = LockBufHdr(buf);
	exact = (buf_state & BM_VALID) != 0
		&& BufferTagsEqual(&handoff->tag, &buf->tag)
		&& cluster_pcm_x_revoke_finish_mode(&buf->tag, 0)
			== CLUSTER_PCM_X_REVOKE_FINISH_DROP
		&& cluster_bufmgr_pcm_current_image_locked(buf, buf_state);
	UnlockBufHdr(buf, buf_state);
	if (!exact)
...
{
	ClusterBufmgrPcmAuxPinHandoff pin_handoff;
	ResourceXAcquisitionRef terminal_ref;
	ResourceXApplyResult result;
	...
	if (!cluster_bufmgr_pcm_aux_pin_handoff_begin(
			buf, &observed->tag, &pin_handoff))
		cluster_bufmgr_pcm_direct_init_report_failure(
			buf, CLUSTER_PCM_OWN_STALE, observed,
	...
	if (!cluster_bufmgr_pcm_aux_pin_handoff_finish_exact(
			buf, &pin_handoff))
	{
		/* No heap/map bytes or BufferDesc state were changed by this
		 * follower.  The old numeric Buffer handle may now name a different
		 * tag, so hand ownership of the retry back to the relation-level
		 * caller.  It will obtain a fresh pin and re-enter the complete
		 * proof/Resource-X path; the stale handle is never dereferenced and a
		 * pre-mutation replacement does not fence the cluster gate. */
...
{
	ClusterBufmgrPcmAuxPinHandoff pin_handoff;
	ClusterPcmDirectInitSnapshot committed_observed;
	ClusterPcmDirectInitSnapshot observed;
	ClusterPcmDirectInitSnapshot pending_observed;
	ClusterPcmOwnSnapshot pending_base;
	ClusterPcmOwnResult pending_result;
	ResourceXAcquisitionRef terminal_ref;
	ResourceXApplyResult resource_x_result;
	ResourceXGateSnapshot gate;
			...
			kind, &observed, proof) == CLUSTER_PCM_OWN_OK)
	{
		...
		UnlockBufHdr(buf, buf_state);
		current_ref = cluster_bufmgr_pcm_join_aux_direct_init_exact(
			buf, kind, &observed, 0);
		if (!current_ref)
		{
			if (pin_replaced == NULL)
			{
				...
				cluster_bufmgr_resource_x_writer_report_failure(
					RESOURCE_X_APPLY_STALE, buf,
					"Resource-X auxiliary direct-init repin");
			}
			*pin_replaced = true;
		}
		return false;
	...
	if (pending_result != CLUSTER_PCM_OWN_OK) {
		if (pending_token != 0)
			cluster_pcm_own_abort_grant_after_error(
				...
				"direct-init pending proof recheck");
		cluster_bufmgr_pcm_direct_init_report_failure(buf, pending_result, &observed,
										  "direct-init consume");
	}
	if (aux_pin_required
		&& !cluster_bufmgr_pcm_aux_pin_handoff_begin(
			buf, &pending_base.tag, &pin_handoff))
	{
		cluster_pcm_own_abort_grant_after_error(
			buf, &pending_base, pending_token,
			"direct-init pin handoff");
		cluster_bufmgr_pcm_direct_init_report_failure(
			buf, CLUSTER_PCM_OWN_STALE, &pending_observed,
			"direct-init pin handoff");
	}
	...
	}
	{
			...
			PG_CATCH();
			{
				cluster_pcm_own_abort_grant_after_error(
					buf, &pending_base, pending_token,
					"direct-init Resource-X acquire");
				PG_RE_THROW();
			}
			PG_END_TRY();
				...
				&& resource_x_result != RESOURCE_X_APPLY_DUPLICATE)
			{
				cluster_pcm_own_abort_grant_after_error(
					buf, &pending_base, pending_token,
					"direct-init Resource-X refusal");
				cluster_bufmgr_resource_x_writer_report_failure(
					resource_x_result, buf,
					"Resource-X direct-init target acquire");
			}
			if (aux_pin_required
				&& !cluster_bufmgr_pcm_aux_pin_handoff_finish_exact(
					buf, &pin_handoff))
			{
				if (pin_replaced == NULL)
				{
					...
					cluster_bufmgr_resource_x_writer_report_failure(
						RESOURCE_X_APPLY_STALE, buf,
						"Resource-X direct-init target repin");
				}
				pin_handoff.active = false;
				*pin_replaced = true;
				return false;
...
static BlockNumber ExtendBufferedRelShared(BufferManagerRelation bmr,
										   ...
										   Buffer *buffers,
										   uint32 *extended_by);
typedef enum PinBufferLookupResult
{
	PIN_BUFFER_LOOKUP_RETRY = 0,
	PIN_BUFFER_LOOKUP_INVALID,
	PIN_BUFFER_LOOKUP_VALID
} PinBufferLookupResult;

static PinBufferLookupResult PinBufferForLookup(
		...
		if ((buf_state & BM_VALID) && BufferTagsEqual(&tag, &bufHdr->tag))
		{
			...
			if (!have_private_ref
				&& !cluster_pcm_x_aux_pin_admission_allowed(
					...
					cluster_bufmgr_should_pcm_track(bufHdr),
					bufHdr->tag.forkNum,
					cluster_pcm_own_flags_get(bufHdr->buf_id)))
			{
...
#ifdef USE_PGRAC_CLUSTER
		cluster_bufmgr_pcm_x_writer_activate_target_direct_init(bufHdr);
#endif
	}
...
retry_lookup:
	...
	if (existing_buf_id >= 0)
	{
		BufferDesc *buf;
		bool		valid;
		PinBufferLookupResult pin_result;
		...
		pin_result = PinBufferForLookup(buf, strategy);

		/* Can release the mapping lock as soon as we've pinned it */
		LWLockRelease(newPartitionLock);
#ifdef USE_PGRAC_CLUSTER
		if (pin_result == PIN_BUFFER_LOOKUP_RETRY)
		{
			(void) cluster_bufmgr_resource_x_wait_retry(
				BufferDescriptorGetContentLock(buf), buf->buf_id,
				aux_pin_waits, NULL);
			if (aux_pin_waits < PG_UINT32_MAX)
				aux_pin_waits++;
			goto retry_lookup;
		}
#endif
		valid = pin_result == PIN_BUFFER_LOOKUP_VALID;
	...
	if (existing_buf_id >= 0)
	{
		...
		pin_result = PinBufferForLookup(existing_buf_hdr, strategy);
		...
		{
			(void) cluster_bufmgr_resource_x_wait_retry(
				BufferDescriptorGetContentLock(existing_buf_hdr),
				existing_buf_hdr->buf_id, aux_pin_waits, NULL);
			if (aux_pin_waits < PG_UINT32_MAX)
				aux_pin_waits++;
			goto retry_lookup;
		}
								 ...
								 LWLock *oldPartitionLock, uint32 buf_state)
{
	...
	release_pcm_holder = cluster_pcm_is_active()
		&& cluster_pcm_x_buffer_tag_tracked(oldTag, cluster_shared_catalog)
...
retry_extend_collision:
		...
		if (existing_id >= 0)
		{
			 ...
			 */
			pin_result = PinBufferForLookup(existing_hdr, strategy);

			LWLockRelease(partition_lock);

#ifdef USE_PGRAC_CLUSTER
			/*
			 * A type-17 auxiliary revoke owns the exact descriptor but has not
			 * changed the mapping yet.  Preserve the native collision polarity:
			 * keep our clean victim, wait off mapping/header/content locks, then
			 * re-run the same insert against current mapping bytes.  The existing
			 * Resource-X gate bounds and interrupts the wait; this creates no new
			 * extension error, authority, or deadline.
			 */
			if (pin_result == PIN_BUFFER_LOOKUP_RETRY)
			{
				(void) cluster_bufmgr_resource_x_wait_retry(
					BufferDescriptorGetContentLock(existing_hdr),
					existing_hdr->buf_id, aux_pin_waits, NULL);
				if (aux_pin_waits < PG_UINT32_MAX)
					aux_pin_waits++;
...
MarkBufferDirty(Buffer buffer)
{
	...
	/*
	 * Content EXCLUSIVE makes this sample stable against every typed image
	 * owner.  Ordinary dirty publication is legal only after target T3 has
...
/* Pin a mapping-table lookup result.  A first passive VM/FSM pin is the one
 * pin entrance that must share the BufferDesc header linearization with an
 * exact REVOKING publication.  Existing private references already contribute
 * one shared refcount and therefore predate the later zero-refcount drain. */
static PinBufferLookupResult
PinBufferForLookup(BufferDesc *buf, BufferAccessStrategy strategy)
{
...
#ifdef USE_PGRAC_CLUSTER
	if (ref == NULL && cluster_pcm_is_active()
		&& cluster_bufmgr_should_pcm_track(buf)
		&& (buf->tag.forkNum == VISIBILITYMAP_FORKNUM
			|| buf->tag.forkNum == FSM_FORKNUM))
	{
		ReservePrivateRefCountEntry();
		buf_state = LockBufHdr(buf);
		if (!cluster_pcm_x_aux_pin_admission_allowed(
				cluster_pcm_is_active(), cluster_bufmgr_should_pcm_track(buf),
				buf->tag.forkNum,
				cluster_pcm_own_flags_get(buf->buf_id)))
		{
			UnlockBufHdr(buf, buf_state);
			return PIN_BUFFER_LOOKUP_RETRY;
		}
		...
		return valid ? PIN_BUFFER_LOOKUP_VALID : PIN_BUFFER_LOOKUP_INVALID;
	}
#endif
	return PinBuffer(buf, strategy)
		? PIN_BUFFER_LOOKUP_VALID : PIN_BUFFER_LOOKUP_INVALID;
}
...
SyncOneBuffer(int buf_id, bool skip_recently_used, WritebackContext *wb_context)
{
	...
	if (!cluster_bufmgr_pcm_aux_pin_admission_locked(bufHdr))
	{
		UnlockBufHdr(bufHdr, buf_state);
		return 0;
	}
			...
			IOContext io_context)
{
	 ...
	 * coherent sidecar observation cannot change before return.  Never enter
	 * StartBufferIO for old transfer bytes, and treat a dirty page behind an
	 * activation fence as invariant damage rather than writing it out.
	 */
	buf_state = LockBufHdr(buf);
	if (cluster_bufmgr_pcm_x_retained_image_locked(buf, buf_state))
	{
	...
	resource_x_activation_generation
		= cluster_pcm_own_resource_x_activation_generation_get(buf->buf_id);
	if (!cluster_pcm_x_flush_fence_consistent(
...
#ifdef USE_PGRAC_CLUSTER
typedef struct ClusterBufmgrBarrierUnwindContext
{
	BufferDesc *buf;
...
static ClusterBufferBarrierCleanupResult
cluster_bufmgr_barrier_abort_writer(void *arg)
{
	ClusterBufmgrBarrierUnwindContext *context = arg;
	ClusterBufferBarrierCleanupResult result;

	result = cluster_bufmgr_pcm_x_writer_abort_acquiring(*context->writer);
...
static ClusterBufferBarrierCleanupResult
cluster_bufmgr_barrier_release_master(void *arg)
{
	ClusterBufmgrBarrierUnwindContext *context = arg;

	cluster_pcm_lock_release_buffer_for_eviction(context->buf, context->pcm_mode);
	return CLUSTER_BUFFER_BARRIER_CLEAN;
}

static ClusterBufferBarrierCleanupResult
cluster_bufmgr_barrier_converge_local(void *arg)
{
	ClusterBufmgrBarrierUnwindContext *context = arg;
	ClusterPcmOwnResult result;

	result = cluster_pcm_own_abort_grant_after_master_rollback(
		context->buf, context->pending_base, context->pending_token);
	return result == CLUSTER_PCM_OWN_OK ? CLUSTER_BUFFER_BARRIER_CLEAN
		: CLUSTER_BUFFER_BARRIER_FAILED;
}

static ClusterBufferBarrierCleanupResult
cluster_bufmgr_barrier_abort_pending(void *arg)
{
	ClusterBufmgrBarrierUnwindContext *context = arg;

	cluster_pcm_own_abort_grant_or_error(context->buf, context->pending_base,
										 context->pending_token,
										 "barrier refusal unwind");
	return CLUSTER_BUFFER_BARRIER_CLEAN;
}
...
static const ClusterBufferBarrierUnwindOps cluster_bufmgr_barrier_unwind_ops = {
	.abort_holder = cluster_bufmgr_barrier_abort_holder,
	.abort_writer = cluster_bufmgr_barrier_abort_writer,
	.release_master = cluster_bufmgr_barrier_release_master,
	.converge_local = cluster_bufmgr_barrier_converge_local,
	.abort_pending = cluster_bufmgr_barrier_abort_pending,
	.prove_empty = cluster_bufmgr_barrier_prove_empty
};

/*
 * PGRAC: the one common clean-refusal epilogue for every
 * barrier-aware LockBufferInternal exit (SHARE and EXCLUSIVE alike).
 *
 * Postconditions on return: this backend holds no target content lock, no
 * ACQUIRING holder/writer ledger entry remains, no durable master grant and
 * no exact GRANT_PENDING reservation taken by this call survives.  A cover
 * that pre-existed the call (cached node residency) is deliberately kept.
 *
 * Master release failure propagates ERROR and leaves the reservation
 * fail-closed.  Local convergence failure after a successful master release
 * raises the existing data-corruption-class ERROR because local and master
 * ownership can no longer be proven equal; that is cleanup failure, never a
 * normal barrier outcome.
 */
static void
cluster_bufmgr_pcm_unwind_barrier_refusal(BufferDesc *buf,
										  PcmLockMode pcm_mode,
										  ClusterPcmOwnSnapshot *pending_base,
										  uint64 pending_token,
										  ...
										  ClusterPcmXWriterLedgerEntry **writer)
{
	...
	if (result != CLUSTER_BUFFER_BARRIER_UNWIND_OK)
		ereport(ERROR,
				(errcode(ERRCODE_DATA_CORRUPTED),
				   ...
				   bool *pcm_x_transient_refused)
{
	BufferDesc *buf;
#ifdef USE_PGRAC_CLUSTER
	BufferTag pcm_expected_resource = {0};
	ClusterBufmgrPcmAuxPinHandoff pcm_aux_pin_handoff = {0};
	bool pcm_acquired = false;
	bool pcm_aux_pin_required = false;
	...
	uint64 pcm_x_absolute_deadline_us = 0;
			 ...
			 && cluster_bufmgr_should_pcm_track(buf))
	{
		...
		if (pcm_mode == PCM_LOCK_MODE_X)
		{
			...
			for (;;)
			{
				if (pcm_aux_pin_required
					&& (!cluster_bufmgr_pcm_aux_pin_handoff_begin(
							buf, &pcm_expected_resource,
							&pcm_aux_pin_handoff)
						|| !BufferTagsEqual(
							&pcm_aux_pin_handoff.tag,
							&pcm_expected_resource)))
					cluster_bufmgr_resource_x_writer_report_failure(
						RESOURCE_X_APPLY_STALE, buf,
						...
						buf, &pcm_aux_pin_handoff))
				{
					if (pcm_pin_replaced != NULL)
					{
						ClusterBufferBarrierCleanupResult cleanup;

						cleanup = cluster_bufmgr_pcm_x_writer_abort_acquiring(
							pcm_x_writer);
						if (cleanup != CLUSTER_BUFFER_BARRIER_CLEAN)
						{
							cluster_bufmgr_resource_x_fail_closed_current();
							cluster_bufmgr_resource_x_writer_report_failure(
								RESOURCE_X_APPLY_BAD_STATE, buf,
								"Resource-X target replacement cleanup");
						}
						pcm_x_writer = NULL;
					...
					cluster_bufmgr_resource_x_writer_report_failure(
						RESOURCE_X_APPLY_STALE, buf,
						"Resource-X target exact repin");
				}
				LWLockAcquire(
					BufferDescriptorGetContentLock(buf), LW_EXCLUSIVE);
				if (cluster_bufmgr_pcm_x_writer_activate(
						pcm_x_writer, true))
					break;
				LWLockRelease(BufferDescriptorGetContentLock(buf));
				pcm_x_writer = NULL;
...
cluster_lockbuffer_barrier_refusal:
			...
			buf, &pcm_aux_pin_handoff))
	{
		...
		cluster_bufmgr_resource_x_writer_report_failure(
			RESOURCE_X_APPLY_STALE, buf,
			"Resource-X barrier exact repin");
	}
...
/*
 * PGRAC: SHARE counterpart with the same clean-refusal
 * contract.  False means exactly one thing: the lower layer returned the
 * process-local BARRIER_CLOSED refusal and the common epilogue proved this
 * backend holds no target content lock and left no holder/writer acquisition
 * or GRANT_PENDING reservation behind; caller-owned outer locks and every
 * pin are untouched.  A pre-existing cached node residency may remain.
 * Local buffers and cluster-disabled builds behave exactly like
 * LockBuffer(..., BUFFER_LOCK_SHARE) and return true.
 */
bool
...
LockBufferForAuxiliaryPageInit(Buffer buffer, ClusterPcmDirectInitKind kind)
{
	...
	if (cluster_pcm_is_active() && cluster_bufmgr_should_pcm_track(buf))
	{
		...
		for (;;)
		{
			...
			{
				if (!cluster_bufmgr_pcm_join_aux_direct_init_exact(
			...
			if (!cluster_bufmgr_pcm_gate_direct_init(
					buf, kind, &direct_init_proof, &pin_replaced))
			{
				if (pin_replaced)
					return InvalidBuffer;
				if (waits < PG_UINT32_MAX)
...
{
#ifdef USE_PGRAC_CLUSTER
	return LockBufferForAuxiliaryPageInit(
		buffer, CLUSTER_PCM_DIRECT_INIT_VM);
#else
	LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
	return buffer;
#endif
...
#ifdef USE_PGRAC_CLUSTER
	return LockBufferForAuxiliaryPageInit(
		buffer, CLUSTER_PCM_DIRECT_INIT_FSM);
#else
	LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
	return buffer;
#endif
	...
	uint64 *out_page_scn)
{
	...
	if (out_page_scn != NULL)
		*out_page_scn = 0;
	if (block_data == NULL || out_page_lsn == NULL || out_page_scn == NULL)
		return false;
	...
	*out_page_lsn = PageGetLSN(page);
	*out_page_scn = (uint64)((PageHeader)page)->pd_block_scn;
	return true;
}
	...
	ClusterBufmgrGcsCopyRefusal *out_refusal)
{
	...
	/* Resource-X precedes BufferContent in the lock order.  If this exact X is
	 * the retained half of a terminal requester episode, keep it authoritative
	 * and let the caller use its existing one-shot read-image path. */
	...
	cover_result
		= cluster_pcm_lock_resource_x_bootstrap_round_x_to_s_preflight_exact(
			&current);
	if (cover_result != RESOURCE_X_APPLY_NOT_FOUND) {
		if (cover_result != RESOURCE_X_APPLY_APPLIED
			&& cover_result != RESOURCE_X_APPLY_BAD_STATE
									   ...
									   BlockNumber blocknum)
{
	 ...
	 */
	pin_result = PinBufferForLookup(buf, NULL);
	LWLockRelease(partition_lock);
	if (pin_result == PIN_BUFFER_LOOKUP_RETRY)
		return InvalidBuffer;
 ...
 *   cluster_bufmgr_lock_resident_for_stamp semantic.  It revalidates the
 *   captured proof under mapping authority, pin, content EXCLUSIVE and the
 *   buffer-header lock before the caller may stamp; every normal mismatch
 *   is a typed safe skip (TT/CLOG/undo stays the terminal authority) and
 *   never triggers storage or Cache Fusion reads.
 * ======================================================================== */
	...
	uint8 *pcm_state)
{
	...
	/*
	 * One header-lock sample of the complete ownership tuple.  The captured
	 * generation must equal the holder identity's base generation: holder
	 * registration already fail-closes when the committed grant generation
	 * diverges from it, so a mismatch here means this content lock round is
	 * not the exact X round the ledger describes.
	 */
	buf_state = LockBufHdr(buf);
	cluster_pcm_own_snapshot_locked(buf, &own);
	header_ok = (buf_state & BM_VALID) != 0
												 ...
												 ClusterItlStampSkipReason *out_reason)
{
	...
	BufferDesc *buf;
	ClusterPcmOwnSnapshot own;
	ClusterItlStampSkipReason reason = CLUSTER_ITL_STAMP_SKIP_NONE;
	PinBufferLookupResult pin_result;
	Page		page;
	...
	*out_reason = CLUSTER_ITL_STAMP_SKIP_NONE;
	...
	/*
	 * A structurally impossible proof is internal corruption, not a normal
	 * skip: registration bounds the slot index and stores only the two
	 * active class values.  Fail closed before the terminal record.
	 */
	if (proof->slot_class != ITL_FLAG_ACTIVE
		&& proof->slot_class != ITL_FLAG_LOCK_ONLY_ACTIVE)
		ereport(ERROR,
				(errcode(ERRCODE_DATA_CORRUPTED),
				 errmsg("cluster ITL terminal-stamp proof is malformed"),
				 errdetail("slot_class=%u buffer=%d", (unsigned) proof->slot_class,
						   proof->buffer_id)));

	InitBufferTag(&tag, &record->key.rloc, record->key.forknum, record->key.block);
	hashcode = BufTableHashCode(&tag);
	partition_lock = BufMappingPartitionLock(hashcode);
	...
	{
		LWLockRelease(partition_lock);
		*out_reason = CLUSTER_ITL_STAMP_SKIP_NOT_RESIDENT;
		return InvalidBuffer;
	}
	buf = GetBufferDescriptor(buf_id);

	/*
	 * A different descriptor for the same tag means the block was evicted
	 * and reloaded since capture: not the exact local X round.  Same-tag
	 * residency is exactly the signal this helper refuses to trust.
	 */
	if (buf_id != proof->buffer_id)
	{
		LWLockRelease(partition_lock);
		*out_reason = CLUSTER_ITL_STAMP_SKIP_OWNERSHIP_GENERATION_CHANGED;
		return InvalidBuffer;
	}

	/* Pin under the partition lock (PinBuffer handles an existing pin). */
	pin_result = PinBufferForLookup(buf, NULL);
	LWLockRelease(partition_lock);
	if (pin_result == PIN_BUFFER_LOOKUP_RETRY)
	{
		*out_reason = CLUSTER_ITL_STAMP_SKIP_OWNERSHIP_FLAGS_BUSY;
		return InvalidBuffer;
	}

	LWLockAcquire(BufferDescriptorGetContentLock(buf), LW_EXCLUSIVE);

	/* One header-lock sample of tag, validity, image shape and ownership. */
	buf_state = LockBufHdr(buf);
	cluster_pcm_own_snapshot_locked(buf, &own);
	header_valid = (buf_state & BM_VALID) != 0;
	header_retained = cluster_bufmgr_pcm_x_retained_image_locked(buf, buf_state);
	...
	/* Page and slot revalidation under the same content lock. */
	page = (Page) BufHdrGetBlock(buf);
	if (!PageHasItl(page) || PageGetSpecialSize(page) < CLUSTER_ITL_ARRAY_SIZE)
		reason = CLUSTER_ITL_STAMP_SKIP_NO_ITL;
	else if (record->key.slot_idx >= CLUSTER_ITL_INITRANS_DEFAULT)
		reason = CLUSTER_ITL_STAMP_SKIP_SLOT_OUT_OF_RANGE;
	else
	{
											   ...
											   &slot->undo_segment_head))
		{
			if (slot->xid != proof->xid)
				...
				reason = CLUSTER_ITL_STAMP_SKIP_ALREADY_TERMINAL;
		}
	}
	...
	ClusterBufmgrGcsCopyRefusal *out_refusal)
{
	...
	/* Keep an exact terminal cached-X episode intact.  This entry lookup is
	 * deliberately before BufferContent; the later content-locked snapshot and
	 * post-commit note retain the existing drift checks. */
	own_result = cluster_bufmgr_pcm_own_snapshot(buf, &current);
	if (own_result != CLUSTER_PCM_OWN_OK || !BufferTagsEqual(&current.tag, &tag)
		|| current.pcm_state != (uint8)PCM_STATE_X || current.flags != 0) {
		cluster_bufmgr_unpin_for_gcs(buf);
...
/* The N and S source paths share one compiler-visible terminal-owner identity.
 * Callers hold content EXCLUSIVE and choose the exact source bytes; this helper
 * revalidates the same REVOKING snapshot under the header lock before the sole
	...
	char block_data[BLCKSZ], XLogRecPtr *out_page_lsn, uint64 *out_page_scn)
{
	...
	if (buf == NULL || expected_revoking == NULL || source_page == NULL
		|| out_revoking == NULL || block_data == NULL || out_page_lsn == NULL
		|| out_page_scn == NULL || expected_revoking->flags != PCM_OWN_FLAG_REVOKING
		|| (expected_revoking->pcm_state != (uint8)PCM_STATE_N
			&& expected_revoking->pcm_state != (uint8)PCM_STATE_S))
		return CLUSTER_PCM_OWN_INVALID;
	Assert(LWLockHeldByMeInMode(BufferDescriptorGetContentLock(buf), LW_EXCLUSIVE));
	...
	if (!cluster_pcm_own_snapshot_matches_locked(buf, expected_revoking)
		...
		result = CLUSTER_PCM_OWN_CORRUPT;
			  ...
			  & (BM_DIRTY | BM_JUST_DIRTIED | BM_CHECKPOINT_NEEDED | BM_IO_IN_PROGRESS)) != 0)
		result = CLUSTER_PCM_OWN_BUSY;
	else {
		if (replace_resident) {
	...
	PG_TRY();
	{
		...
		if (result == CLUSTER_PCM_OWN_OK && LWLockConditionalAcquire(content_lock, LW_EXCLUSIVE)) {
			...
			result = cluster_bufmgr_pcm_own_copy_source_image_exact(
				buf, &live, (Page)scratch.data, true, out_revoking, block_data,
				out_page_lsn, out_page_scn);
			LWLockRelease(content_lock);
			content_locked = false;
			...
			if (!cluster_pcm_own_snapshot_matches_locked(buf, &live)
				...
				*out_refusal = CLUSTER_PCM_OWN_SOURCE_PREPARE_REFUSAL_IO_IN_PROGRESS;
				...
				if (!local_covers && !storage_covers) {
					...
					selected_page = local_page;
				} else {
					selected_page = storage_page;
...
static bool
cluster_bufmgr_pcm_own_held_x_revoke_valid(
	const ClusterPcmOwnHeldXRevoke *held)
{
	return held != NULL
		&& held->flags == CLUSTER_PCM_OWN_HELD_X_REVOKE_KNOWN_MASK
		&& held->buffer_id >= 0
		&& held->buffer_id < NBuffers
		&& held->revoking.pcm_state == (uint8)PCM_STATE_X
		&& held->revoking.flags == PCM_OWN_FLAG_REVOKING
	...
	ClusterPcmOwnHeldXRevoke *held_out)
{
	...
	uint32 buf_state;
	int buffer_id;
	bool caller_pinned = false;

	if (held_out != NULL)
		memset(held_out, 0, sizeof(*held_out));
	if (tag == NULL || expected_x == NULL || held_out == NULL
		|| !BufferTagsEqual(tag, &expected_x->tag)
		|| expected_x->pcm_state != (uint8)PCM_STATE_X
		|| expected_x->flags != 0
	...
	partition_lock = BufMappingPartitionLock(hashcode);
	LWLockAcquire(partition_lock, LW_SHARED);
	buffer_id = BufTableLookup(&lookup_tag, hashcode);
	if (buffer_id < 0) {
		LWLockRelease(partition_lock);
		return CLUSTER_PCM_OWN_STALE;
	}
	...
	memset(&revoking, 0, sizeof(revoking));
	result = cluster_bufmgr_pcm_own_begin_x_revoke(
		buf, expected_x, &revoking);
	if (result != CLUSTER_PCM_OWN_OK) {
		cluster_bufmgr_unpin_for_gcs(buf);
		return result;
	}
	Assert(caller_pinned);
...
/* Pre-arm unwind: restore the exact X first, then drop this call's pin.  A
 * failed abort keeps both handle and pin so the caller can fail-close before
 * using the explicit abandon boundary below. */
ClusterPcmOwnResult
	...
	ClusterPcmOwnHeldXRevoke *held)
{
	...
	result = cluster_bufmgr_pcm_own_abort_x_revoke(
		buf, &held->revoking);
	if (result != CLUSTER_PCM_OWN_OK)
		return result;
	cluster_bufmgr_unpin_for_gcs(buf);
...
/* Before the type-17 caller copies or publishes anything, prove without
 * waiting that every content holder predating REVOKING has drained.  The
 * caller's raw pin keeps descriptor identity stable; this helper changes no
 * ownership state and never performs I/O.  BUSY must be returned to the LMON
 * event loop instead of retried here, because a content-S holder may itself
 * be waiting for a visibility reply delivered by that loop. */
ClusterPcmOwnResult
...
/* VM/FSM do not retain a descriptor image.  After REVOKING has atomically
 * closed first-pin admission, prove that every passive predecessor pin has
 * drained before any retained status/image pair is published.  This helper
 * waits for nothing, performs no I/O, and changes no ownership state. */
ClusterPcmOwnResult
	...
	BufferDesc *buf, const ClusterPcmOwnSnapshot *expected_revoking)
{
	...
	if (buf == NULL || expected_revoking == NULL
		...
		|| expected_revoking->reservation_token == 0
		|| cluster_pcm_x_revoke_finish_mode(
			&expected_revoking->tag, 0) != CLUSTER_PCM_X_REVOKE_FINISH_DROP)
		return CLUSTER_PCM_OWN_INVALID;
...
/* The runtime must already be fail-closed.  Do not mutate REVOKING or the
 * retained pair; release only this call's untracked raw pin.  Snapshot drift
 * remains CORRUPT but cannot justify leaking the shared refcount. */
ClusterPcmOwnResult
	...
	ClusterPcmOwnHeldXRevoke *held)
{
	...
	result = cluster_pcm_own_snapshot_matches_locked(
		...
		&& (buf_state & BM_VALID) != 0
		&& cluster_bufmgr_pcm_current_image_locked(buf, buf_state)
		? CLUSTER_PCM_OWN_OK : CLUSTER_PCM_OWN_CORRUPT;
	UnlockBufHdr(buf, buf_state);
	cluster_bufmgr_unpin_for_gcs(buf);
	memset(held, 0, sizeof(*held));
	return result;
...
/* SourceSettlementV2 proves only that the exact immutable transfer carrier no
 * longer needs its REVOKING fence.  It is not durable-current coverage and
 * therefore must preserve the established N+PI/BM_VALID recovery image even
		...
		else
		{
			...
			if (result == CLUSTER_PCM_OWN_OK)
			{
				*release_applied_out = true;
				...
				if (buf->buffer_type != (uint8) BUF_TYPE_PI
					|| buf->pcm_state != (uint8) PCM_STATE_N
					|| (buf_state & BM_VALID) == 0)
					result = CLUSTER_PCM_OWN_CORRUPT;
				else
 ...
 *   published retained-image authority.  REVOKING also closes the gate unless
 *   this caller already holds BufferContent EXCLUSIVE and the descriptor is
 *   the exact X predecessor of the pre-retention drain.  That predecessor must
 *   finish its current ITL/page mutation so the event-loop drain observes
 *   BUSY, aborts the exact revoke, and replays; every new entrant remains
 *   fenced.  Outside those explicit states,
...
cluster_bufmgr_block_write_permitted(Buffer buffer)
{
	...
	bool		content_x_held;
	bool		retained_image;
	bool		revoking_predecessor;

	if (BufferIsLocal(buffer))
		return true;			/* temp / local buffers are never CF-shared */
	buf = GetBufferDescriptor(buffer - 1);
	content_x_held = LWLockHeldByMeInMode(BufferDescriptorGetContentLock(buf),
										LW_EXCLUSIVE);
	buf_state = LockBufHdr(buf);
	...
	revoking_predecessor = content_x_held
		&& cluster_pcm_x_content_holder_mutation_allowed(
			cluster_pcm_is_active(), cluster_bufmgr_should_pcm_track(buf),
			retained_image, buf->pcm_state, own_flags,
			cluster_pcm_own_writer_activation_token_get(buf->buf_id),
			cluster_pcm_own_resource_x_activation_generation_get(buf->buf_id));
	if (retained_image
		|| ((own_flags & PCM_OWN_FLAG_REVOKING) != 0
			&& !revoking_predecessor))
	{
		UnlockBufHdr(buf, buf_state);
		return false;
	}
