...
#ifdef USE_PGRAC_CLUSTER
	/*
	 * PGRAC spec-4.5 §3.3d (rules 2 + round-8 P1): clear PD_CLUSTER_FORCE_FPI
	 * ONLY now -- the insert succeeded (EndPos valid) AND a real
	 * BKPIMAGE_APPLY image was emitted.  Doing it here (not in assemble)
	 * survives assemble re-entry / InvalidXLogRecPtr retries; a failed
	 * insert leaves the bit set.
	 */
	{
		int			i;
		...
		for (i = 0; i < max_registered_block_id; i++)
		{
			...
			if (rb->in_use && rb->force_fpi_applied)
				 ...
				 * bit, spec-4.5 A-closure); 4.5a's gcs_block_install_block
