	...
	uint64 r4_record_generation)
{
	...
	ResourceXLocalOwnerHandle resource_x_owner;
	...
	bool resource_x_owner_held = false;
	...
	memset(&resource_x_owner, 0, sizeof(resource_x_owner));
	...
	if (finish_required) {
		if (MyProc == NULL || MyProc->pgprocno < 0) {
			failure_result = RESOURCE_X_APPLY_RECOVERY_BLOCKED;
			goto pre_retained_failure;
		}
		owner_result = cluster_pcm_lock_resource_x_source_revoke_enter_exact(
			block, authenticated_master_node, r4_record_generation,
			current.generation, current.reservation_token,
			(uint32)MyProc->pgprocno, &resource_x_owner);
	...
	if (resource_x_owner_held
		&& cluster_pcm_lock_resource_x_source_revoke_revalidate_exact(
			&resource_x_owner) != RESOURCE_X_APPLY_APPLIED) {
		failure_result = RESOURCE_X_APPLY_RECOVERY_BLOCKED;
		goto pre_retained_failure;
	}
	if (!semantic_retained) {
