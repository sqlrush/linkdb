	...
	uint16 plans_cap, uint16 *plan_count)
{
		...
		for (i = 0; i < nmembers; i++) {
			...
			if (plan == NULL
				|| plan->request.prefix.entry_count
					   == CLUSTER_CURRENT_MX_MAX_PROOF_ASKS_PER_FRAME) {
				plan = &plans[built++];
				wire_init_proof_request(
					plan, origin, key, descriptor_hash, nmembers, request_id, current_epoch,
					requester_node, requester_backend_id,
	...
	if (challenge != NULL) {
		...
		wire_init_proof_request(
			plan, member_origin_nodes[updater_ordinal], key, descriptor_hash, nmembers, request_id,
						...
						uint64 current_epoch)
{
	...
	case CCM_ACTIVE:
		/*
		 * A SUBCOMMITTED member may resolve to an ACTIVE parent xid.  The
		 * member echo remains the child; the full key names the holder.
		 */
