...
/*
 * S3-P0-15: consume sealed native-era CLOG before interpreting an ITL ref.
 *
 * A cluster-off seed transaction predates every striped allocation even when
 * a pg_basebackup joiner later sees an exact-looking origin-0 ITL ref for the
 * tuple xid.  Such a transaction has no cluster-era live TT overlay.  Waiting
 * until `ref->local_xid != raw_xid` therefore misroutes exact seed refs to the
 * remote-live path and fails closed forever.
 *
 * This helper is deliberately not a general local-CLOG escape.  It returns a
 * verdict only while the boot-verified native-prehistory latch proves the full
 * xid is below the sealed native high-water and the CLOG segment still covers
 * it.  The reader/wrap-barrier drain and XactTruncationLock ordering are the
 * same as the former recycled-ref-only consume.  SUB_COMMITTED and every
 * uncertain leg return false to the existing fail-closed resolver.
 */
static bool
resolve_native_prehistory(TransactionId raw_xid, ClusterVisResolve *out)
{
	bool prehistory_resolved = false;

	if (out == NULL || cluster_cr_native_prehistory_covered_hw() == 0)
		return false;
				  ...
				  SCN read_scn, ClusterVisResolve *out)
{
	...
	/*
	 * P0-15: the sealed era outranks the apparent ref owner.  In particular,
	 * an exact origin-0 ref on a basebackup joiner is native prehistory, not a
	 * remote live TT identity.  Every unproved leg falls through unchanged.
	 */
	if (resolve_native_prehistory(raw_xid, out))
		return;
	...
	if (out->status == CLUSTER_TT_STATUS_UNKNOWN && cluster_crossnode_runtime_visibility
		...
		default: {
			...
			ClusterUndoVerdictResult v = cluster_undo_verdict_resolve(
				(int)ref->origin_node_id, (uint32)ref->undo_segment_id,
				raw_xid, (uint32)ref->tt_slot_id, read_scn,
