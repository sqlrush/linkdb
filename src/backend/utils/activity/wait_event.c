...
pgstat_get_wait_cluster_interconnect(WaitEventCluster w)
{
	...
	case WAIT_EVENT_CLUSTER_IC_RDMA_POLL:
		...
		break;
	case WAIT_EVENT_INTERCONNECT_RDMA_BUSYPOLL:
		event_name = "InterconnectRdmaBusypoll";
		break;
	case WAIT_EVENT_INTERCONNECT_RDMA_INLINE_SEND:
	...
	case WAIT_EVENT_CLUSTER_IC_RECONNECT:
		...
		break;
	case WAIT_EVENT_CLUSTER_LMS_DATA_RECV:
		event_name = "ClusterLmsDataRecv";
		break;
	case WAIT_EVENT_CLUSTER_LMS_DATA_SEND:
		event_name = "ClusterLmsDataSend";
		break;
	default:
		break;
	}
...
pgstat_get_wait_cluster_bgproc(WaitEventCluster w)
{
	...
	case WAIT_EVENT_CLUSTER_BGPROC_CLUSTER_STATS_MAIN_LOOP:
		...
		break;
	case WAIT_EVENT_CLUSTER_BGPROC_UNDO_CLEANER_MAIN_LOOP:
		event_name = "ClusterBgProcUndoCleanerMainLoop";
		break;
