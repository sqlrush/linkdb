...
typedef enum {
	...
	/* PGRAC: spec-3.6b — requester waits for immutable member-list,
	 * origin-authoritative member-proof, and per-node statistics replies. */
	WAIT_EVENT_GCS_MULTIXACT_DESCRIBE_WAIT,
	WAIT_EVENT_GCS_MULTIXACT_MEMBER_PROOF_WAIT,
	...
	/* spec-5.18 D12: the permanent-removal coordinator (LMON) waits for all
	...
	/* Cluster: Interconnect (7 events + 6 spec-2.2 D8) -- AD-007 */
		...
		WAIT_EVENT_INTERCONNECT_RDMA_BUSYPOLL,
		WAIT_EVENT_INTERCONNECT_RDMA_INLINE_SEND,
		WAIT_EVENT_CLUSTER_IC_RDMA_CONNECT,
	...
	/* PGRAC: spec-7.2 D6 -- LMS data-plane loop waits. */
	WAIT_EVENT_CLUSTER_LMS_DATA_RECV,
