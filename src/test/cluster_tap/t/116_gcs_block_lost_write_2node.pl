...
#	  L9  pg_cluster_state.gcs category has 119 keys (PCM-X queue observability +3)
...
is($pair->node0->safe_psql('postgres',
	...
	'124',
	'L2 pg_stat_cluster_wait_events returns 124 rows (including current-DML MultiXact describe wait; merge sum 118+3+2+1)');

is($pair->node0->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '119',
   'L2 pg_cluster_state.gcs category has 119 keys (PCM-X queue observability +3)');
...
# ============================================================
# L9 (alias of L2): gcs key count = 119.
# ============================================================
is($pair->node1->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '119',
   'L9 node1 pg_cluster_state.gcs has 119 keys (PCM-X queue observability +3)');
