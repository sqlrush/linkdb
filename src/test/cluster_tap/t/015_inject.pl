...
is( $node->safe_psql('postgres',
	...
	'186',	'pg_stat_cluster_injections returns the exact 186-row registry including the CTRC stage barrier');
...
# ----------
# Test 5b: prove the exact R2 seam can hold skipn:2 state, then use the
# SQL-triggerable debug seam to verify the generic process-local countdown and
# explicit-disarm semantics.  The R2 seam itself is fired end-to-end in t/407.
# ----------
my $skipn_states = $node->safe_psql('postgres', q{
	CREATE TEMP TABLE r2_inject_state
		(seq int, label text, fault_type text, param bigint, hits bigint);
	DO $$BEGIN
		PERFORM cluster_inject_fault(
			'cluster-pcm-share-barrier-refuse-after-acquire', 'skipn', 2);
	END$$;
	INSERT INTO r2_inject_state
		SELECT 1, 'armed', fault_type, param, hits
		  FROM pg_stat_cluster_injections
		 WHERE name = 'cluster-pcm-share-barrier-refuse-after-acquire';
	DO $$BEGIN
		PERFORM cluster_inject_fault(
			'cluster-pcm-share-barrier-refuse-after-acquire', 'none', 0);
		PERFORM cluster_inject_fault('cluster-debug-dump-entry', 'skipn', 2);
	END$$;
	DO $$BEGIN
		PERFORM count(*) FROM cluster_dump_state();
		PERFORM count(*) FROM cluster_dump_state();
	END$$;
	INSERT INTO r2_inject_state
		SELECT 2, 'consumed', fault_type, param, hits
		  FROM pg_stat_cluster_injections
		 WHERE name = 'cluster-debug-dump-entry';
	DO $$BEGIN
		PERFORM cluster_inject_fault('cluster-debug-dump-entry', 'none', 0);
	END$$;
	INSERT INTO r2_inject_state
		SELECT 3, 'disarmed', fault_type, param, hits
		  FROM pg_stat_cluster_injections
		 WHERE name = 'cluster-debug-dump-entry';
	DO $$BEGIN
		PERFORM count(*) FROM cluster_dump_state();
	END$$;
	INSERT INTO r2_inject_state
		SELECT 4, 'stable', fault_type, param, hits
		  FROM pg_stat_cluster_injections
		 WHERE name = 'cluster-debug-dump-entry';
	SELECT label || '|' || fault_type || '|' || param || '|' || hits
	  FROM r2_inject_state ORDER BY seq;
});
my @skipn_rows = split /\n/, $skipn_states;
is($skipn_rows[0], 'armed|skipn|2|0',
	'R2 seam arms skipn:2 with zero initial hits in the same backend');
is($skipn_rows[1], 'consumed|skipn|2|2',
	'skipn:2 consumes exactly two dispatches in one backend');
my ($consumed_hits) = $skipn_rows[1] =~ /\|(\d+)$/;
is($skipn_rows[2], "disarmed|none|0|$consumed_hits",
	'generic skipn point explicitly disarms after countdown consumption');
is($skipn_rows[3], "stable|none|0|$consumed_hits",
	'generic skipn point remains disarmed with no later hit drift');
...
is( $node->safe_psql('postgres',
	...
	'123',
	'pg_stat_cluster_wait_events returns 123 rows (spec-7.2 LMS data-plane waits + spec-5.22b D2-6 undo-block grant-plane waits; merge sum 118+2+3)');
