...
is($node->safe_psql(
   ...
   '183',
   'L12a pg_stat_cluster_injections is 183 (branch-1 +2; full breakdown in t/015)');

is($node->safe_psql(
		'postgres',
		'SELECT count(*) FROM pg_stat_cluster_wait_events'),
   '124',
   'L12b pg_stat_cluster_wait_events returns 124 rows (including current-DML MultiXact describe wait; merge sum 118+3+2+1)');
