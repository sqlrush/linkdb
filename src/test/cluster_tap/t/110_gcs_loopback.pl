...
#	  L4  CLUSTER_WAIT_EVENTS_COUNT == 125 (including current-DML MultiXact waits)
...
# L4 — CLUSTER_WAIT_EVENTS_COUNT == 125.
my $total_wait_events = $node->safe_psql(
	'postgres', 'SELECT count(*) FROM pg_stat_cluster_wait_events');
is($total_wait_events, '125',
	'L4 wait_events count 125 (including current-DML MultiXact waits)');
