...
UT_TEST(test_r11_lockbuffer_writer_selector_is_single_ingress_choice_and_exclusive)
{
	 ...
	 * revalidations.  No source or legacy branch remains selectable. */
	UT_ASSERT_EQ(count_occurrences(
		source, "cluster_resource_x_writer_path_snapshot("), 3);
	UT_ASSERT_NULL(strstr(source, "cluster_bufmgr_pcm_x_writer_prepare_source("));
	UT_ASSERT_NULL(strstr(source, "case RESOURCE_X_WRITER_SOURCE:"));
...
UT_TEST(test_resource_x_target_writer_context_is_post_t3_and_local_cleanup_only)
{
	...
	const char *cleanup_starts[3];
	const char *cleanup_ends[3];
	char *source = read_bufmgr_source();
	size_t i;
	size_t j;
	...
	UT_ASSERT_NULL(strstr(source, "ResourceXWriterPath writer_path"));
	...
	assert_ordered_in_function(
		...
		"\n/* D11 target-native BTR seam.",
		abort_contract, lengthof(abort_contract));
	assert_ordered_in_function(
		source, "\ncluster_bufmgr_pcm_x_writer_exception_cleanup_all(",
		"\nstatic void\ncluster_bufmgr_pcm_x_owner_exit_drain(",
		exception_contract, lengthof(exception_contract));
	UT_ASSERT_NOT_NULL(strstr(source,
		"cluster_bufmgr_pcm_x_owner_exit_drain(void)\n{\n\tcluster_bufmgr_pcm_x_writer_exception_cleanup_all();"));

	cleanup_starts[0] = strstr(source, "\ncluster_bufmgr_pcm_x_writer_release(");
	cleanup_ends[0] = cleanup_starts[0] != NULL
		? strstr(cleanup_starts[0], "\nstatic ClusterBufferBarrierCleanupResult") : NULL;
	cleanup_starts[1] = strstr(source, "\ncluster_bufmgr_pcm_x_writer_abort_acquiring(");
	cleanup_ends[1] = cleanup_starts[1] != NULL
		? strstr(cleanup_starts[1], "\n/* D11 target-native BTR seam.") : NULL;
	cleanup_starts[2] = strstr(source,
		"\ncluster_bufmgr_pcm_x_writer_exception_cleanup_all(");
	cleanup_ends[2] = cleanup_starts[2] != NULL
		? strstr(cleanup_starts[2], "\nstatic void\ncluster_bufmgr_pcm_x_owner_exit_drain(") : NULL;
	for (i = 0; i < lengthof(cleanup_starts); i++) {
		UT_ASSERT_NOT_NULL(cleanup_starts[i]);
		UT_ASSERT_NOT_NULL(cleanup_ends[i]);
		if (cleanup_starts[i] == NULL || cleanup_ends[i] == NULL)
			continue;
		for (j = 0; j < lengthof(cleanup_forbidden); j++) {
			const char *site = strstr(cleanup_starts[i], cleanup_forbidden[j]);

			UT_ASSERT(site == NULL || site >= cleanup_ends[i]);
		}
	}
	free(source);
}
