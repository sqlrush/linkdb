...
{
	reset_fake_pcm_runtime(4);
	UT_ASSERT_EQ(fake_pcm_entrysize, 888);
...
UT_TEST(test_resource_x_bootstrap_round_fans_in_and_retries_same_attempt)
{
	...
	ResourceXLocalOwnerHandle competing_recycler;
	ResourceXLocalOwnerHandle source_revoke;
	ResourceXLocalOwnerHandle resumed_source_revoke;
	ResourceXLocalOwnerHandle stale_source_revoke;
	...
	/* BTR-INV-20..23 survives D11 source removal as an exact local-only
	 * Resource-X owner.  A cached-X follower may not reuse the terminal ref
	 * while the resolver is off content-X; exact release wakes it and restores
	 * the original post-T3 cover without changing acquisition identity. */
	memset(&recycler, 0, sizeof(recycler));
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_recycler_arm_exact(
		&expected_ref, 77, 91, 0, 7, &recycler),
		RESOURCE_X_APPLY_APPLIED);
	UT_ASSERT_EQ(recycler.kind, RESOURCE_X_LOCAL_OWNER_RECYCLING);
	UT_ASSERT_EQ(recycler.owner_procno, UINT32_C(7));
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_recycler_revalidate_exact(
		&recycler), RESOURCE_X_APPLY_APPLIED);
	memset(&competing_recycler, 0, sizeof(competing_recycler));
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_recycler_arm_exact(
		&expected_ref, 77, 91, 0, 8, &competing_recycler),
		RESOURCE_X_APPLY_BAD_STATE);
	memset(&source_block, 0, sizeof(source_block));
	source_block.kind = RESOURCE_X_WIRE_BLOCK_TO_N;
	source_block.common.base_authority_generation = 9;
	source_block.common.authority_generation = 9;
	source_block.common.resource_formation = 17;
	source_block.common.master_session_incarnation = 31;
	source_block.common.assertion_sequence = 2;
	source_block.common.action_node = 1;
	source_block.common.observed_mode = PCM_STATE_X;
	source_block.common.target_mode = PCM_STATE_N;
	source_block.common.source_candidate = 1;
	source_block.common.retain_pi_if_dirty = 1;
	source_block.common.sender_connection_generation = 61;
	UT_ASSERT(resource_x_assertion_init(
		&tag, 2, &source_block.common.logical_assertion));
	fake_gcs_master_node = 0;
	memset(&source_revoke, 0, sizeof(source_revoke));
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_source_revoke_enter_exact(
		&source_block, 0, 77, 91, 5, 9, &source_revoke),
		RESOURCE_X_APPLY_BAD_STATE);
	action = cluster_pcm_lock_resource_x_bootstrap_round_step_exact(
		&assertion, 0, 17, 31, 77, 51, 61,
		UINT64_C(2000), UINT64_C(220), UINT64_C(50),
		true, 91, &retry_assertion, &terminal_ref);
	UT_ASSERT_EQ(action, RESOURCE_X_BOOTSTRAP_ROUND_WAIT);
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_recycler_release_exact(
		&recycler), RESOURCE_X_APPLY_APPLIED);
	UT_ASSERT_EQ(recycler.kind, RESOURCE_X_LOCAL_OWNER_EMPTY);
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_source_revoke_enter_exact(
		&source_block, 0, 77, 91, 5, 9, &source_revoke),
		RESOURCE_X_APPLY_APPLIED);
	UT_ASSERT_EQ(source_revoke.kind, RESOURCE_X_LOCAL_OWNER_REVOKING);
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_source_revoke_revalidate_exact(
		&source_revoke), RESOURCE_X_APPLY_APPLIED);
	action = cluster_pcm_lock_resource_x_bootstrap_round_step_exact(
		&assertion, 0, 17, 31, 77, 51, 61,
		UINT64_C(2000), UINT64_C(220), UINT64_C(50),
		true, 91, &retry_assertion, &terminal_ref);
	UT_ASSERT_EQ(action, RESOURCE_X_BOOTSTRAP_ROUND_WAIT);
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_source_revoke_release_exact(
		&source_revoke), RESOURCE_X_APPLY_APPLIED);
	/* The selected S carrier uses the authenticated type-17 plus its exact
	 * BufferDesc lineage as a local-only mutex.  It does not need or create a
	 * second GRD holder authority, but it must exclude the same local writer
	 * round until the physical S revoke reaches terminal. */
	source_block.common.observed_mode = PCM_STATE_S;
	memset(&source_revoke, 0, sizeof(source_revoke));
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_source_revoke_enter_exact(
		&source_block, 0, 77, 93, 12, 9, &source_revoke),
		RESOURCE_X_APPLY_APPLIED);
	UT_ASSERT_EQ(source_revoke.kind, RESOURCE_X_LOCAL_OWNER_REVOKING);
	UT_ASSERT_EQ(source_revoke.source_mode, (uint8)PCM_STATE_S);
	UT_ASSERT_EQ(source_revoke.current_ref.acquisition_generation,
		UINT64_C(0));
	UT_ASSERT_EQ(source_revoke.resource_formation, UINT64_C(17));
	UT_ASSERT_EQ(source_revoke.r4_record_generation, UINT64_C(77));
	UT_ASSERT_EQ(source_revoke.buffer_ownership_generation, UINT64_C(93));
	UT_ASSERT_EQ(source_revoke.reservation_token, UINT64_C(12));
	UT_ASSERT_EQ(source_revoke.base_authority_generation, UINT64_C(9));
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_source_revoke_revalidate_exact(
		&source_revoke), RESOURCE_X_APPLY_APPLIED);
	/* A post-retain finish failure loses only the process-local copy of the
	 * handle.  The same actor may recover the exact stored owner; a competing
	 * actor cannot take it over or select a new source. */
	memset(&resumed_source_revoke, 0, sizeof(resumed_source_revoke));
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_source_revoke_enter_exact(
		&source_block, 0, 77, 93, 12, 9, &resumed_source_revoke),
		RESOURCE_X_APPLY_APPLIED);
	UT_ASSERT(memcmp(&resumed_source_revoke, &source_revoke,
		sizeof(source_revoke)) == 0);
	memset(&resumed_source_revoke, 0, sizeof(resumed_source_revoke));
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_source_revoke_enter_exact(
		&source_block, 0, 77, 93, 12, 10, &resumed_source_revoke),
		RESOURCE_X_APPLY_BAD_STATE);
	UT_ASSERT_EQ(resumed_source_revoke.kind,
		RESOURCE_X_LOCAL_OWNER_EMPTY);
	stale_source_revoke = source_revoke;
	stale_source_revoke.buffer_ownership_generation++;
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_source_revoke_revalidate_exact(
		&stale_source_revoke), RESOURCE_X_APPLY_STALE);
	action = cluster_pcm_lock_resource_x_bootstrap_round_step_exact(
		&assertion, 0, 17, 31, 77, 51, 61,
		UINT64_C(2000), UINT64_C(220), UINT64_C(50),
		false, 0, &retry_assertion, &terminal_ref);
								 ...
								 int32 source_node)
{
	...
	blocked.body.blocked_to_n.source_fence[28] = PCM_STATE_X;
...
UT_TEST(test_resource_x_adapter_head_rebinds_only_before_assert)
{
	...
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_adapter_head_rebind_exact(
		&assertion.common.logical_assertion, 41, 73, 17, 5, &authority),
		RESOURCE_X_APPLY_STALE);
								...
								ResourceXDecodedFrame *image)
{
	...
	*grant = make_resource_x_master_frame(
	...
	grant->body.authority_grant.source_fence[28] = PCM_STATE_X;
...
UT_TEST(test_resource_x_x_source_defers_self_master_grd_transition_to_ingress)
{
	...
	/* A later direct-current-X lineage may replace the drained tombstone, but
	 * only with a strictly newer admission and physical carrier generation.
	 * The old DRAIN remains replayable after that replacement. */
	newer_block = block;
	newer_status = status;
	newer_image = image;
	newer_block.common.assertion_sequence = 42;
	newer_status.common.assertion_sequence = 42;
	newer_status.body.blocked_to_n.source_carrier_generation = 62;
	newer_status.body.blocked_to_n.requester_target_generation = 42;
	newer_image.common.assertion_sequence = 42;
	newer_image.body.image_envelope.source_carrier_generation = 62;
	newer_image.body.image_envelope.requester_target_generation = 42;
	 ...
	 * but it is not evidence for this newer admission.  Classify the current
	 * attempt as having no pair yet so the source can build fresh frames; the
	 * existing replacement path still requires the predecessor's exact DRAIN
	 * tombstone and a strictly newer carrier before it changes any owner. */
	UT_ASSERT_EQ(cluster_pcm_lock_resource_x_holder_pair_publish_needed_exact(
