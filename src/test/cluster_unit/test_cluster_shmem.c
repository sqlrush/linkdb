...
/* spec-5.1b D7: cluster_ges_mode_selfcheck GUC removed; cluster_guc.o no
 * longer references it, so the stub definition is gone. */
 ...
 * cluster_gcs_module_init (cluster_gcs.c). */
void
...
/* Spec-2.34 D2 stub: cluster_init_shmem_module also calls
 * cluster_gcs_block_dedup_module_init (cluster_gcs_block_dedup.c). */
void
...
/* spec-5.4 D1 stub: cluster_init_shmem_module also calls
 * cluster_sequence_shmem_register (cluster_sequence_shmem.c); the unit
 * harness does not link that object, so provide a no-op. */
void
cluster_sequence_shmem_register(void)
{}

/* spec-6.14 D6 stub: cluster_init_shmem_module also calls
 * cluster_oid_lease_shmem_register (cluster_oid_lease_shmem.c); the unit
 * harness does not link that object, so provide a no-op. */
void
...
/* spec-5.59 D1 stub: cluster_init_shmem_module also calls
 * cluster_xnode_profile_shmem_register (cluster_xnode_profile.c); the unit
 * harness does not link that object, so provide a no-op. */
void
...
/* spec-6.12b stub: cluster_init_shmem_module also calls
 * cluster_cr_server_shmem_register (cluster_cr_server.c); no-op here. */
void
...
/* spec-3.6b D4 stub: current-MultiXact authority counters use an independent
 * region; this standalone harness does not link the stats object. */
void
