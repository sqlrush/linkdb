 ...
 *
 * test_cluster_diag.c
 *	  Compile-time / link-level invariants for spec-1.13 DIAG Sprint A.
 *
 *	  Locks:
 *	    - ClusterDiagStatus enum values (NOT_STARTED=0, SPAWNING=1,
 *	      READY=2, SHUTTING_DOWN=3, EXITED=4) are frozen.
 *	    - ClusterDiagSharedState size stays under 4 KiB (catch
 *	      accidental field bloat early).
 *	    - cluster_diag_status_to_string() returns non-null for every
 *	      enum value and "(unknown)" for out-of-range.
 *	    - Public symbols cluster_diag_start / wait_for_ready /
 *	      request_shutdown / status / shmem_register/init / DiagMain
 ...
 *
 * IDENTIFICATION
 *	  src/test/cluster_unit/test_cluster_diag.c
 *
 * NOTES
...
/* ----------
 * Stubs needed to link cluster_diag.o standalone.  Runtime paths
 * (DiagMain / shmem init) are not exercised here; these are address-
 ...
 * here, but cluster_diag_start() is a thin proxy that forwards to it,
...
/* Spec-1.11 Sprint B: cluster_diag.c references
 * cluster_diag_main_loop_interval GUC + WaitLatch / ResetLatch /
 * MyLatch + cluster_inject framework.  Stubs cover them all --
 * runtime DiagMain is not exercised. */
...
/* libpq + procsignal stubs (pulled in transitively via cluster_diag.c
 * includes; DiagMain runtime is not invoked). */
struct sigaction;
...
/* Sprint B: Latch / WaitLatch / ResetLatch stubs (DiagMain runtime
...
/* cluster_diag.c references MyBackendType (set by DiagMain). */
#include "miscadmin.h"
...
UT_TEST(test_diag_status_enum_values_frozen)
{
	UT_ASSERT_EQ((int)CLUSTER_DIAG_NOT_STARTED, 0);
	UT_ASSERT_EQ((int)CLUSTER_DIAG_SPAWNING, 1);
	UT_ASSERT_EQ((int)CLUSTER_DIAG_READY, 2);
	UT_ASSERT_EQ((int)CLUSTER_DIAG_SHUTTING_DOWN, 3);
	UT_ASSERT_EQ((int)CLUSTER_DIAG_EXITED, 4);
	UT_ASSERT_EQ((int)CLUSTER_DIAG_STATUS_LAST, 4);
}
...
UT_TEST(test_diag_status_to_string_lookup)
{
	int i;

	for (i = 0; i <= (int)CLUSTER_DIAG_STATUS_LAST; i++) {
		const char *s = cluster_diag_status_to_string((ClusterDiagStatus)i);
...
UT_TEST(test_diag_status_unknown_returns_unknown)
{
	const char *neg = cluster_diag_status_to_string((ClusterDiagStatus)-1);
	const char *over
		= cluster_diag_status_to_string((ClusterDiagStatus)((int)CLUSTER_DIAG_STATUS_LAST + 1));
...
UT_TEST(test_diag_public_symbols_linkable)
{
	UT_ASSERT_NOT_NULL((void *)cluster_diag_start);
	UT_ASSERT_NOT_NULL((void *)cluster_diag_wait_for_ready);
	UT_ASSERT_NOT_NULL((void *)cluster_diag_request_shutdown);
	UT_ASSERT_NOT_NULL((void *)cluster_diag_status);
	UT_ASSERT_NOT_NULL((void *)cluster_diag_status_to_string);
	UT_ASSERT_NOT_NULL((void *)cluster_diag_shmem_size);
	UT_ASSERT_NOT_NULL((void *)cluster_diag_shmem_init);
	UT_ASSERT_NOT_NULL((void *)cluster_diag_shmem_register);
	UT_ASSERT_NOT_NULL((void *)DiagMain);
}
