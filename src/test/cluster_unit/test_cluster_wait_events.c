...
UT_TEST(test_last_event_per_category_in_class)
{
	...
	UT_ASSERT_EQ(((uint32)WAIT_EVENT_GCS_MULTIXACT_STATS_WAIT) & 0xFF000000U,
				 PG_WAIT_CLUSTER_PCM);
...
/* ----------
 * Per-category event counts match the design doc roster
 *  (GES 5, PCM 27, BufferShip 5, SCN 4, Reconfig selected range 7,
...
UT_TEST(test_per_category_event_counts)
{
	...
	UT_ASSERT_EQ((uint32)WAIT_EVENT_GCS_MULTIXACT_STATS_WAIT
	 ...
	 * (spec-3.11) + BUF_FLUSH + EXTENT_CLAIM (spec-3.18 D7) + BLOCK_GRANT_WAIT +
	 * BLOCK_INVALIDATE_WAIT + BLOCK_REMASTER_WAIT (spec-5.22b D2-6) = 11.  Count
	 * the full range to the last event so future additions are caught here (F12). */
	UT_ASSERT_EQ(
		(uint32)WAIT_EVENT_UNDO_BLOCK_REMASTER_WAIT - (uint32)WAIT_EVENT_UNDO_REMOTE_READ + 1, 11);
...
main(void)
{
	...
	UT_RUN(test_current_multixact_wait_events_are_contiguous);
	UT_DONE();
