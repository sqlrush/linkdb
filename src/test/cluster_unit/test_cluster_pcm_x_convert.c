...
{
	UT_ASSERT_EQ(PCM_X_SHMEM_LAYOUT_VERSION, 18);
	...
	UT_ASSERT_EQ(sizeof(PcmXStats), 256);
	UT_ASSERT_EQ(sizeof(PcmXStatsSnapshot), 320);
	...
	UT_ASSERT_EQ(sizeof(PcmXLocalMembershipSlot), 240);
	...
	UT_ASSERT_EQ(offsetof(PcmXLocalMembershipSlot, retry_terminal), 168);
	...
	UT_ASSERT_EQ(offsetof(PcmXShmemHeader, outbound_targets), 35456);
	...
	UT_ASSERT_EQ(offsetof(PcmXShmemHeader, acquire_observation), 36592);
	UT_ASSERT_EQ(sizeof(PcmXShmemHeader), 36992);
}
...
UT_TEST(test_layout_v18_preserves_transfer_terminal_and_retry_observation)
{
	PcmXShmemLayout layout;

	cluster_pcm_x_layout_compute(122, 25, 16384, 1024, &layout);
	UT_ASSERT_EQ(layout.version, 18);
...
UT_TEST(test_local_writer_claim_runs_closed_cohort_and_blocks_next_round)
{
	...
	UT_ASSERT_EQ(cluster_pcm_x_local_writer_claim_exact(&follower, &claim), PCM_X_QUEUE_NOT_FOUND);
...
UT_TEST(test_local_completed_follower_detaches_before_terminal_drain)
{
	...
	init_active_pcm_x(UINT64_C(77));
	bind_local_master(1, UINT64_C(9), UINT64_C(177));
	identity = make_wait_identity(808, 0, 22, UINT64_C(80801));
	UT_ASSERT_EQ(cluster_pcm_x_local_join_begin(&identity, 1, UINT64_C(177), &leader),
				 PCM_X_QUEUE_OK);
	identity = make_wait_identity(808, 0, 23, UINT64_C(80802));
	UT_ASSERT_EQ(cluster_pcm_x_local_join_begin(&identity, 1, UINT64_C(177), &follower),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_follower_wfg_snapshot_exact(&follower, &wfg), PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_follower_wfg_commit_exact(&wfg, UINT64_C(903)),
	...
	UT_ASSERT_EQ(tag_slot->closed_round_member_count, 1);
	UT_ASSERT_EQ(tag_slot->head_index, leader.membership_slot.slot_index);
	UT_ASSERT_EQ(tag_slot->tail_index, leader.membership_slot.slot_index);
	UT_ASSERT_EQ(tag_slot->leader_index, leader.membership_slot.slot_index);
	UT_ASSERT_EQ(tag_slot->active_writer_index, PCM_X_INVALID_SLOT_INDEX);
	UT_ASSERT_EQ(ClusterPcmXConvertShmem->allocator[PCM_X_ALLOC_LOCAL_WAIT].used, 1);
	UT_ASSERT_EQ(
		cluster_pcm_x_directory_find(PCM_X_DIR_LOCAL_WAIT, &follower.identity, &found),
		PCM_X_DIRECTORY_NOT_FOUND);
}
...
UT_TEST(test_local_writer_claim_completion_is_fifo_and_one_shot)
{
	...
	UT_ASSERT_EQ(cluster_pcm_x_local_writer_claim_exact(&follower1, &claim), PCM_X_QUEUE_NOT_FOUND);
	UT_ASSERT_EQ(cluster_pcm_x_local_cancel_exact(&follower1, NULL), PCM_X_QUEUE_NOT_FOUND);
	...
	UT_ASSERT_EQ(cluster_pcm_x_local_writer_claim_release_collect_exact(&claim, &successor),
				 PCM_X_QUEUE_OK);
	UT_ASSERT(memcmp(&successor, &(PcmXWaitIdentity){ 0 }, sizeof(successor)) == 0);
	UT_ASSERT_EQ(cluster_pcm_x_local_writer_claim_exact(&follower2, &claim), PCM_X_QUEUE_NOT_FOUND);
	UT_ASSERT_EQ(cluster_pcm_x_local_cancel_exact(&leader, NULL), PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_detach_terminal_exact(&leader), PCM_X_QUEUE_OK);
	assert_local_queue_baseline(ClusterPcmXConvertShmem);
}
										   ...
										   uint64 leader_rekey_base)
{
	...
	if (leader_rekey_base != 0) {
		PcmXLocalHandle rekeyed;

		UT_ASSERT_EQ(cluster_pcm_x_local_leader_rekey_generation_exact(
						 &fixture->writer, leader_rekey_base, &rekeyed),
					 PCM_X_QUEUE_OK);
		fixture->writer = rekeyed;
	}
	UT_ASSERT_EQ(cluster_pcm_x_local_writer_claim_exact(&fixture->writer, &fixture->writer_claim),
				 PCM_X_QUEUE_OK);
	...
	UT_ASSERT_EQ(cluster_pcm_x_local_enqueue_arm_exact(&fixture->writer, &enqueue, &token),
				 PCM_X_QUEUE_OK);
	memset(&fixture->admit_ack, 0, sizeof(fixture->admit_ack));
	fixture->admit_ack.ref.identity = fixture->writer.identity;
...
prepare_local_rebase_fixture(BlockNumber block, uint64 master_session,
							 TestLocalRebaseFixture *fixture)
{
	prepare_local_rebase_fixture_with_follower(block, master_session, fixture, NULL, 0);
}
...
UT_TEST(test_local_follower_claim_inherits_published_rebase)
{
	...
	prepare_local_rebase_fixture_with_follower(7122, master_session, &fixture, &follower, 0);
	UT_ASSERT_EQ(cluster_pcm_x_local_grant_rebase_publish_exact(&fixture.writer, UINT64_C(8)),
				 PCM_X_QUEUE_OK);
...
/* A leader may be rekeyed to the current ownership generation immediately
 * before ENQUEUE while an already-joined follower keeps its immutable local
 * identity.  The later follower claim must bind to the completed node grant,
 * not fall back to that stale member-local base. */
UT_TEST(test_local_follower_claim_inherits_rekeyed_leader_grant_base)
{
	TestLocalRebaseFixture fixture;
	PcmXInstallReadyPayload install_ready;
	PcmXPhasePayload commit;
	...
	prepare_local_rebase_fixture_with_follower(7124, master_session, &fixture, &follower,
											  UINT64_C(8));
	UT_ASSERT_EQ(fixture.tag_slot->grant_base_own_generation, 0);

	UT_ASSERT_EQ(cluster_pcm_x_local_install_ready_arm_exact(&fixture.writer,
													 &fixture.prepare.ref,
													 &fixture.prepare.image, &install_ready,
													 &token),
				 PCM_X_QUEUE_OK);
	...
	UT_ASSERT_EQ(cluster_pcm_x_local_writer_claim_exact(&follower, &follower_claim),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(follower_claim.writer.identity.base_own_generation, UINT64_C(5));
	UT_ASSERT_EQ(follower_claim.grant_base_own_generation, UINT64_C(8));
	memset(&granted, 0, sizeof(granted));
	granted.tag = follower_claim.writer.identity.tag;
	granted.generation = UINT64_C(9);
...
UT_TEST(test_resource_x_retry_submission_is_visible_to_bounded_work_cursor)
{
	...
	UT_ASSERT(cluster_pcm_x_stats_snapshot(&snapshot));
	UT_ASSERT_EQ(snapshot.master_grant_delivery_pending_count, 1);
	UT_ASSERT(snapshot.master_grant_delivery_oldest_age_us > 0);
	...
	UT_ASSERT(cluster_pcm_x_stats_snapshot(&snapshot));
	UT_ASSERT_EQ(snapshot.retry_transport_rebound_count, 0);
	transport.connection_generation = 10;
	UT_ASSERT_EQ(resource_x_retry_classify_exact(&state, &state.attempt, &transport,
											 UINT64_C(3000), &action),
				 RESOURCE_X_RETRY_STAGE_EXACT);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_admitted_exact(
					 &work.local_handle, &work.local_token, &action, UINT64_C(5000), &state),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(state.retry_count, 2);
	UT_ASSERT_EQ(state.state_generation, 3);
	UT_ASSERT(cluster_pcm_x_stats_snapshot(&snapshot));
	UT_ASSERT_EQ(snapshot.retry_transport_rebound_count, 1);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_admitted_exact(
...
UT_TEST(test_resource_x_retry_exhaustion_is_atomic_and_post_no_return_refuses)
{
	...
	PcmXStatsSnapshot before;
	PcmXStatsSnapshot after;
	Size cursor;
	...
	UT_ASSERT_EQ(cluster_pcm_x_local_enqueue_arm_exact(&leader, &enqueue, &token),
				 PCM_X_QUEUE_OK);
	UT_ASSERT(cluster_pcm_x_stats_snapshot(&before));
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_submission_admitted_exact(
	...
	UT_ASSERT(cluster_pcm_x_stats_snapshot(&after));
	UT_ASSERT_EQ(after.retry_budget_exhausted_count,
				 before.retry_budget_exhausted_count + 1);
	UT_ASSERT_EQ(after.retry_terminal_denied_count, before.retry_terminal_denied_count);
	UT_ASSERT_EQ(after.retry_terminal_latency_us_count,
				 before.retry_terminal_latency_us_count + 1);
	UT_ASSERT_EQ(after.retry_terminal_latency_us_max, UINT64_C(1000));
	UT_ASSERT_EQ(after.master_grant_delivery_pending_count, 0);
	UT_ASSERT_EQ(UINT64_C(1),
				 after.retry_budget_exhausted_count - before.retry_budget_exhausted_count
					 + after.master_grant_delivery_pending_count
					 - before.master_grant_delivery_pending_count);
	...
	tag_slot = &local_tag_slots(header)[leader.tag_slot.slot_index];
	members = membership_slots(header);
	resource_x_terminal_record_clear(
		&members[leader.membership_slot.slot_index].retry_terminal);
...
UT_TEST(test_resource_x_retry_recovery_blocked_is_exact_and_idempotent)
{
	PcmXShmemHeader *header;
	PcmXLocalHandle leader;
	PcmXWaitIdentity identity;
	...
	init_active_pcm_x(UINT64_C(77));
	header = ClusterPcmXConvertShmem;
	identity = make_wait_identity(763, 0, 16, UINT64_C(70024));
	identity.base_own_generation = UINT64_C(32);
	bind_local_master(1, identity.cluster_epoch, UINT64_C(7334));
	UT_ASSERT_EQ(cluster_pcm_x_local_join_begin(&identity, 1, UINT64_C(7334), &leader),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_enqueue_arm_exact(&leader, &enqueue, &token),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_submission_admitted_exact(
				 ...
				 PCM_X_QUEUE_OK);
	UT_ASSERT(cluster_pcm_x_stats_snapshot(&before));
	UT_ASSERT_EQ(before.master_grant_delivery_pending_count, 1);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_recovery_blocked_exact(
					 &leader, &token, &expected, UINT64_C(2000), &blocked),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(blocked.last_phase, RESOURCE_X_RETRY_PHASE_RECOVERY_BLOCKED);
	UT_ASSERT_EQ(blocked.terminal_errcode, 0);
	UT_ASSERT_EQ(blocked.state_generation, expected.state_generation + 1);
	UT_ASSERT(cluster_pcm_x_stats_snapshot(&after));
	UT_ASSERT_EQ(after.retry_recovery_blocked_count,
				 before.retry_recovery_blocked_count + 1);
	UT_ASSERT_EQ(after.retry_terminal_latency_us_count,
				 before.retry_terminal_latency_us_count + 1);
	UT_ASSERT_EQ(after.master_grant_delivery_pending_count, 0);
	UT_ASSERT_EQ(UINT64_C(0),
				 after.retry_recovery_blocked_count - before.retry_recovery_blocked_count
					 + after.master_grant_delivery_pending_count
					 - before.master_grant_delivery_pending_count);
	cursor = header->layout.pools[PCM_X_POOL_MASTER_TAG].capacity
		+ leader.tag_slot.slot_index;
	UT_ASSERT_EQ(cluster_pcm_x_retry_work_next(&cursor, 1, &work),
				 PCM_X_QUEUE_NOT_FOUND);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_recovery_blocked_exact(
					 &leader, &token, &expected, UINT64_C(3000), &blocked),
				 PCM_X_QUEUE_DUPLICATE);
	UT_ASSERT(cluster_pcm_x_stats_snapshot(&after));
	UT_ASSERT_EQ(after.retry_recovery_blocked_count,
				 before.retry_recovery_blocked_count + 1);
...
UT_TEST(test_resource_x_terminal_record_replays_and_promotes_successor)
{
	...
	init_active_pcm_x(UINT64_C(77));
	header = ClusterPcmXConvertShmem;
	leader_identity = make_wait_identity(765, 0, 8, UINT64_C(70016));
	follower_identity = make_wait_identity(765, 0, 9, UINT64_C(70017));
	leader_identity.base_own_generation = UINT64_C(24);
	follower_identity.base_own_generation = UINT64_C(25);
	bind_local_master(1, leader_identity.cluster_epoch, UINT64_C(7555));
	UT_ASSERT_EQ(cluster_pcm_x_local_join_begin(&leader_identity, 1, UINT64_C(7555), &leader),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_join_begin(&follower_identity, 1, UINT64_C(7555), &follower),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_writer_claim_exact(&leader, &claim), PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_begin_revoke_cutoff_exact(&leader, &cutoff),
				 PCM_X_QUEUE_DUPLICATE);
	...
	tag_slot = &local_tag_slots(header)[leader.tag_slot.slot_index];
	members = membership_slots(header);
	UT_ASSERT(resource_x_retry_state_is_clear(&tag_slot->retry_state));
	UT_ASSERT(resource_x_terminal_record_replay(
		&members[leader.membership_slot.slot_index].retry_terminal, &expected.attempt, &replay));
	UT_ASSERT(memcmp(&replay, &terminal, sizeof(replay)) == 0);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_state_exact(&leader, &replay), PCM_X_QUEUE_OK);
	UT_ASSERT(memcmp(&replay, &terminal, sizeof(replay)) == 0);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_exhausted_exact(
					 &leader, &token, &expected, UINT64_C(3000), &replay),
				 PCM_X_QUEUE_DUPLICATE);
	UT_ASSERT(memcmp(&replay, &terminal, sizeof(replay)) == 0);

	memset(&wake, 0x7f, sizeof(wake));
	UT_ASSERT_EQ(cluster_pcm_x_local_writer_claim_release_collect_exact(&claim, &wake),
				 PCM_X_QUEUE_OK);
	UT_ASSERT(memcmp(&wake, &(PcmXWaitIdentity){ 0 }, sizeof(wake)) == 0);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_terminal_complete_exact(&leader, &promoted),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(test_slot_state(&members[leader.membership_slot.slot_index].slot),
				 PCM_XL_CANCELLED);
	UT_ASSERT_EQ(test_slot_state(&members[follower.membership_slot.slot_index].slot),
				 PCM_XL_NODE_LEADER);
	UT_ASSERT(memcmp(&promoted.identity, &follower_identity, sizeof(follower_identity)) == 0);
	UT_ASSERT_EQ(promoted.membership_slot.slot_index,
				 follower.membership_slot.slot_index);
	UT_ASSERT_EQ(promoted.membership_slot.slot_generation,
				 follower.membership_slot.slot_generation);
	UT_ASSERT(resource_x_retry_state_is_clear(&tag_slot->retry_state));
	UT_ASSERT_EQ(tag_slot->reliable.pending_opcode, 0);
	UT_ASSERT_EQ(tag_slot->prehandle.prehandle_sequence, 0);
	UT_ASSERT_EQ(cluster_pcm_x_local_detach_terminal_exact(&leader), PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(tag_slot->attempt_base_generation,
				 follower_identity.base_own_generation);
	UT_ASSERT_EQ(cluster_pcm_x_local_lookup_exact(&leader_identity, &leader),
				 PCM_X_QUEUE_NOT_FOUND);
	UT_ASSERT_EQ(cluster_pcm_x_local_lookup_exact(&follower_identity, &follower),
				 PCM_X_QUEUE_OK);
}

UT_TEST(test_resource_x_specific_type60_denial_is_exact_and_idempotent)
{
	PcmXLocalHandle leader;
	PcmXLocalHandle promoted;
	PcmXWaitIdentity identity;
	PcmXEnqueuePayload enqueue;
	PcmXAdmitAckPayload admit_ack;
	PcmXPhasePayload terminal_ack;
	PcmXLocalReliableToken token;
	ResourceXRetryStateV1 state;
	ResourceXRetryStateV1 terminal;
	PcmXStatsSnapshot before;
	PcmXStatsSnapshot after;
	const uint64 master_session = UINT64_C(7666);

	init_active_pcm_x(UINT64_C(77));
	identity = make_wait_identity(766, 0, 10, UINT64_C(70018));
	identity.base_own_generation = UINT64_C(26);
	bind_local_master(1, identity.cluster_epoch, master_session);
	UT_ASSERT_EQ(cluster_pcm_x_local_join_begin(&identity, 1, master_session, &leader),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_enqueue_arm_exact(&leader, &enqueue, &token),
	...
	admit_ack.ref.handle.queue_generation = UINT64_C(12);
	admit_ack.prehandle = enqueue.prehandle;
	admit_ack.result = PCM_X_QUEUE_OK;
	admit_ack.phase = PCM_X_LOCAL_RELIABLE_PHASE_ENQUEUE;
	UT_ASSERT_EQ(cluster_pcm_x_local_apply_admit_ack_exact(
					 &leader, &admit_ack, 1, master_session),
				 PCM_X_QUEUE_OK);
	MemSet(&terminal_ack, 0, sizeof(terminal_ack));
	terminal_ack.ref = admit_ack.ref;
	terminal_ack.reason = ERRCODE_CLUSTER_GCS_BLOCK_INVALIDATE_TIMEOUT;
	terminal_ack.phase = PCM_X_LOCAL_RELIABLE_PHASE_CANCEL;
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_terminal_ack_exact(
					 &leader, &terminal_ack, 1, master_session, UINT64_C(2000), &terminal),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(terminal.terminal_errcode,
				 ERRCODE_CLUSTER_GCS_BLOCK_INVALIDATE_TIMEOUT);
	UT_ASSERT(cluster_pcm_x_stats_snapshot(&after));
	UT_ASSERT_EQ(after.retry_terminal_denied_count,
				 before.retry_terminal_denied_count + 1);
	UT_ASSERT_EQ(after.retry_budget_exhausted_count, before.retry_budget_exhausted_count);
	UT_ASSERT_EQ(after.retry_terminal_latency_us_count,
				 before.retry_terminal_latency_us_count + 1);
	UT_ASSERT_EQ(after.retry_terminal_latency_us_max, UINT64_C(1000));
	UT_ASSERT_EQ(after.master_grant_delivery_pending_count, 0);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_terminal_ack_exact(
					 &leader, &terminal_ack, 1, master_session, UINT64_C(3000), &state),
				 PCM_X_QUEUE_DUPLICATE);
	UT_ASSERT(memcmp(&state, &terminal, sizeof(state)) == 0);
	UT_ASSERT(cluster_pcm_x_stats_snapshot(&after));
	UT_ASSERT_EQ(after.retry_terminal_denied_count,
				 before.retry_terminal_denied_count + 1);
	UT_ASSERT_EQ(after.retry_terminal_latency_us_count,
...
UT_TEST(test_resource_x_terminal_replays_to_coalesced_attempt_before_successor)
{
	PcmXShmemHeader *header;
	PcmXLocalMembershipSlot *members;
	PcmXLocalHandle leader;
	...
	ResourceXRetryStateV1 terminal;
	ResourceXRetryStateV1 replay;
	const uint64 master_session = UINT64_C(7777);

	init_active_pcm_x(UINT64_C(77));
	header = ClusterPcmXConvertShmem;
	members = membership_slots(header);
	identity = make_wait_identity(767, 0, 11, UINT64_C(70019));
	identity.base_own_generation = UINT64_C(31);
	bind_local_master(1, identity.cluster_epoch, master_session);
	UT_ASSERT_EQ(cluster_pcm_x_local_join_begin_semantic(
					 &identity, 1, master_session, &leader, &join_result),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(join_result, RESOURCE_X_LOCAL_LEADER_MUST_SUBMIT);
	identity.procno++;
	identity.xid++;
	...
	identity.wait_seq++;
	UT_ASSERT_EQ(cluster_pcm_x_local_join_begin_semantic(
					 &identity, 1, master_session, &follower, &join_result),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(join_result, RESOURCE_X_LOCAL_JOINED_LOCAL_ASSERTION);
	UT_ASSERT_EQ(cluster_pcm_x_local_writer_claim_exact(&leader, &claim), PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_begin_revoke_cutoff_exact(&leader, &cutoff),
	...
	UT_ASSERT_EQ(cluster_pcm_x_local_join_begin_semantic(
					 &identity, 1, master_session, &successor, &join_result),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(join_result, RESOURCE_X_LOCAL_WAIT_SUCCESSOR_ROUND);
	UT_ASSERT_EQ(successor.local_round, cutoff.next_round);
	...
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_exhausted_exact(
					 &leader, &token, &expected, UINT64_C(2000), &terminal),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_state_exact(&follower, &replay), PCM_X_QUEUE_OK);
	UT_ASSERT(memcmp(&replay, &terminal, sizeof(replay)) == 0);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_terminal_ready_exact(&leader),
				 PCM_X_QUEUE_NOT_READY);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_terminal_ready_exact(&follower),
				 PCM_X_QUEUE_NOT_READY);
	UT_ASSERT(resource_x_terminal_record_is_clear(
		&members[successor.membership_slot.slot_index].retry_terminal));
	memset(&wake, 0x7f, sizeof(wake));
	UT_ASSERT_EQ(cluster_pcm_x_local_writer_claim_release_collect_exact(&claim, &wake),
				 PCM_X_QUEUE_OK);
	UT_ASSERT(memcmp(&wake, &follower.identity, sizeof(wake)) == 0);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_terminal_ready_exact(&follower), PCM_X_QUEUE_OK);

	UT_ASSERT_EQ(cluster_pcm_x_local_cancel_exact(&follower, NULL), PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_detach_terminal_exact(&follower), PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_terminal_ready_exact(&leader), PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_terminal_complete_exact(&leader, &promoted),
				 PCM_X_QUEUE_OK);
	UT_ASSERT_EQ(cluster_pcm_x_local_detach_terminal_exact(&leader), PCM_X_QUEUE_OK);
	UT_ASSERT(slot_refs_equal(promoted.membership_slot, successor.membership_slot));
	UT_ASSERT_EQ(cluster_pcm_x_local_retry_state_exact(&promoted, &replay),
				 PCM_X_QUEUE_NOT_READY);
}
...
UT_TEST(test_resource_x_activation_refuses_open_local_round)
{
	...
	UT_ASSERT_EQ(cluster_pcm_x_allocator_release_exact(
				 ...
				 PCM_X_SLOT_RESERVED_NONVISIBLE), PCM_X_ALLOC_OK);
}
...
/*
 * implementation review (2026-08-20): a failed tag-epoch advance (allocator/view
...
main(void)
{
	...
	UT_RUN(test_layout_v18_preserves_transfer_terminal_and_retry_observation);
	...
	UT_RUN(test_local_follower_claim_inherits_rekeyed_leader_grant_base);
