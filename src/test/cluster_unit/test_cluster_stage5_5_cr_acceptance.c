 ...
 *	    L6  CLUSTER_WAIT_EVENTS_COUNT snapshot = 124, including the
 *	        current-DML MultiXact describe wait; future additions must bump
 *	        this snapshot and all dump/test baselines in lockstep.
 *
...
UT_TEST(test_stage5_5_wait_events_count_snapshot_124)
{
	 ...
	 * spec-7.2 D6 adds 2 LMS DATA-plane waits. */
...
main(void)
{
	...
	UT_RUN(test_stage5_5_wait_events_count_snapshot_124);
	UT_DONE();
}
