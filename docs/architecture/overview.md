...
For the four-node two-stage acceptance substrate, coordinated clean shutdown,
direct-I/O voting-device transition, failure cleanup, and the validation order
from resource reuse through the performance baseline, see
[四节点两阶段验收基板](four-node-two-stage-acceptance/README.md).
...
The `cluster_ic` module defines five tier slots.  The TCP tier is active
by default for multi-node heartbeat traffic.  The RDMA tier slots are
compiled only with `--with-rdma`; without that build option, selecting
`tier2` or `tier3` fails closed with `53R22`.
...
| `tier3` | **RDMA tier3 shipped in spec-6.13** — prefers mlx5 direct-verbs capability when available, keeps generic verbs/TCP fallback according to `cluster.interconnect_rdma_fallback`, and includes the GCS block-reply direct-land lane. |
