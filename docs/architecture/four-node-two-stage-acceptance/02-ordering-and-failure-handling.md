...
flowchart TD
    ...
    F --> G{子进程与 FD<br/>是否退出?}
    G -- 是 --> H[逆序解绑已知 device]
    G -- 不可中断 I/O --> M[写持久清理清单<br/>保留 device]
    M --> R[稍后显式 reaper<br/>复核后回收]
    R --> H
...
| 进程陷入不可中断 I/O | 留存精确进程/设备清单，返回失败；等进程和 FD 消失后显式回收 | 重复发信号、延长超时、强行解绑或把重启环境算成功 |
...
Linux 进程处于 `D` 状态时，通常正在等待内核 I/O 完成。此时重复 `pg_ctl` 或 `kill -9` 不保证立即生效；若强行解绑仍被进程持有的 loop device，可能破坏证据并把清理动作变成新的故障源。

安全做法是：

1. 只发起一次有界的既定失败收束；
2. 用 PID 和进程启动时间共同标识原进程，避免 PID 复用误伤；
3. 到达绝对截止时间后记录进程状态、等待点、相关 FD 和 device/backing 映射；
4. 把待回收对象写入测试目录之外的持久清理清单；
5. 保留 device，不在进程或 FD 仍存在时解绑；
6. 下次运行前或由 operator 显式调用 reaper，重新核对进程已消失、holder 为空、设备身份未漂移，再按绑定逆序解绑。

宿主环境重启只能是 operator 的最后恢复手段。发生重启的测试轮次仍然失败，重启不能替代证据、不能计作自动清理成功。

## 9. Reaper 只能回收自己记录的对象

显式 reaper 不能扫描并解绑全部 `/dev/loop*`。每个待回收项至少绑定：

- 测试 attempt 标识；
- 设备路径和 major/minor；
- backing file 的规范化绝对路径；
- 绑定顺序；
- 相关进程的 PID 与启动时间。

任一身份不一致时停止回收并报告冲突。只有全部 live process 和 FD holder 消失后，才允许逆序解绑；重复执行必须幂等。
